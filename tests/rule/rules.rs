...
// ================================================
// ✅ 测行为不测实现：是 — 断言验证公式和幂等性属性
// ✅ 符合领域规则：是 — 覆盖伤害公式、属性叠加、标签位运算
// ✅ 确定性：是 — proptest 使用固定 seed 或充分采样
// ✅ 使用标准数据：是 — 使用标准属性/标签数据
// ✅ 无越界测试：是 — 仅测试公共 API
// ✅ 未测试私有实现：是 — 仅通过 calculate_damage/Attributes/GameplayTags 接口测试
// ================================================
