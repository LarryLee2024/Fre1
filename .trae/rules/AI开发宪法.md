...
- 🟩 Blanket Impl 自动派生：当一个能力可由另一个能力自动推导时，必须使用 blanket impl，禁止手动重复实现
- 🟩 Marker Trait 模式：允许使用空 Marker Trait 驱动自动注册系统（如 DomainEvent、ReplayEvent），Marker Trait 不携带行为，不违反"组合优于继承"
...
1. ❌ 把Entity当面向对象实例
2. ❌ 把Resource当全局变量仓库
3. ❌ 创建全局顶层`systems.rs/components.rs`巨文件（Domain 内部按需拆分，详见 `architecture-constitution.md` §3.4）
4. ❌ 滥用事件系统模拟函数调用
5. ❌ 业务逻辑直接操作UI
6. ❌ 直接修改最终属性值
7. ❌ 为单个实现创建Trait
...
### 类型注册规范
- 🟩 所有新类型注册必须使用 `register_domain_types!` 宏，禁止手动调用 `register_type`
- 宏自动处理类型注册、Reflect 注册和序列化注册，减少遗漏风险
- 手动 register_type 仅限第三方库类型或宏无法覆盖的边界场景

### 集成访问规范
- 🟩 跨 Capability 集成访问必须使用 `ReadFacade` / `WriteFacade` 模式
- 禁止在 Domain 或 Capability 内部直接通过裸 Query 访问其他模块的 Component
- Facade 层位于 `integration/` 子目录，提供类型安全、可测试的访问接口

### 可解释性要求
- 🟨 所有复杂计算（属性公式、伤害计算、状态转换、概率判定）应考虑实现 `Explain` trait
- `Explain` 提供逐步推导输出，支持 Debug 调试和 Battle Replay 验证
- 计算公式变更时，Explain 输出须同步更新，保证推导链可追溯
