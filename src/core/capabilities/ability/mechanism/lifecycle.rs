...
mod tests {
    use super::*;
    use crate::core::capabilities::ability::foundation::ActivationContext;
    ...
    #[test]
    fn unit_001_activate_instant_ability_success() {
    ...
    #[test]
    fn unit_002_activate_casting_ability_success() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_003_reject_activation_on_cooldown() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_004_reject_duplicate_active_instance() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
    ...
    #[test]
    fn unit_005_activate_with_costs() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_010_transition_ready_to_active() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_011_transition_casting_to_ready() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_012_invalid_transition_ready_to_cooldown() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
        ...
        // Ready → Cooldown is invalid
        let result = transition_to(&mut container, &id, AbilityState::Cooldown);
        assert!(result.is_err());
    }

    #[test]
    fn unit_013_transition_to_removed_from_any_state() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_020_cancel_casting_returns_to_ready() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_021_cancel_active_removes() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_022_cancel_ready_is_error() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();

        // Transition from Active → Ready (via cancel) then try again
        cancel_ability(&mut container, &id).unwrap();

        // Now it's Removed, can't cancel again
    ...
    #[test]
    fn unit_030_complete_ability_enters_cooldown() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_031_complete_ability_no_cooldown() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_032_complete_casting_ability_is_error() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_040_tick_cooldown_expires() {
        let mut container = ActiveAbilityContainer::empty();
        container.set_cooldown(CooldownEntry::new("abl_001", 2));
    ...
    #[test]
    fn unit_041_force_reset_cooldown() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_042_start_cooldown_zero_turns_noop() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_050_apply_block_pauses_active_abilities() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_051_remove_block_restores_state() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_052_block_casting_restores_to_casting() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_060_advance_cast_progress() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request_casting("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    #[test]
    fn unit_061_advance_cast_on_instant_ability_is_error() {
        let mut container = ActiveAbilityContainer::empty();
        let request = make_request("abl_001", "abl_000001", 1);
        let id = try_activate(&mut container, request).unwrap();
    ...
    // ── Ready abilities query ──────────────────────────────

    #[test]
    fn unit_070_get_ready_abilities() {
        let mut container = ActiveAbilityContainer::empty();
        start_cooldown(&mut container, "abl_001", 2);
        // abl_002 is the active one
        ...
        // abl_001 is on cooldown, abl_002 is active, abl_003 is ready
    ...
    #[test]
    fn unit_080_start_multiple_cooldowns() {
        let mut container = ActiveAbilityContainer::empty();
    ...
    #[test]
    fn unit_090_ability_error_display() {
    ...
    #[test]
    fn unit_091_instance_id_display() {
