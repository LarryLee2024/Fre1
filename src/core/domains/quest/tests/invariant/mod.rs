...
fn progress_never_decreases() {
    assert!(
        !check_progress_monotonic(3, 5),
        "decreasing progress must be invalid"
    );
}
...
fn reward_granted_only_once() {
    ...
    assert!(
        is_reward_already_granted(&entry),
        "completed → reward marked as granted"
    );
}
...
fn reward_not_granted_for_incomplete() {
    ...
    assert!(
        !is_reward_already_granted(&entry),
        "non-completed → reward not granted"
    );
}
...
fn critical_quest_protected() {
    assert!(
        !can_abandon_quest(true),
        "critical quest cannot be abandoned"
    );
}
...
fn completed_is_terminal() {
    assert!(
        !can_transition(&QuestState::Completed, &QuestState::Active),
        "completed → active blocked"
    );
    assert!(
        !can_transition(&QuestState::Completed, &QuestState::Failed),
...
fn make_entry() -> QuestEntry {
    let objectives = vec![ObjectiveDef {
