// 地图数据：数据驱动的地形定义 + 关卡配置
// 支持从 content/terrains/*.ron 和 content/stages/*.ron 外部配置文件加载
...
impl RegistryLoader for TerrainRegistry {
    ...
    fn register_item(&mut self, item: TerrainDefRon) {
        ...
        bevy::log::info!(target: "map", event = "terrain_loaded", id = %id, "地形已加载");
    }
                ...
                match std::fs::read(&path) {
                    Ok(bytes) => match ron::de::from_bytes::<LevelConfigDef>(&bytes) {
                        Ok(item) => {
                            ...
                            bevy::log::info!(target: "map", event = "level_loaded", id = %id, "关卡已加载");
                            loaded = true;
                        }
...
impl RegistryLoader for LevelRegistry {
    ...
    fn register_item(&mut self, item: LevelConfigDef) {
        ...
        bevy::log::info!(target: "map", event = "level_loaded", id = %id, "关卡已加载（无 TerrainRegistry）");
    }
...
impl Plugin for MapDataPlugin {
    fn build(&self, app: &mut App) {
        let mut terrain_registry = TerrainRegistry::load_from_dir("content/terrains");
        terrain_registry.register_defaults();

        let level_registry =
            LevelRegistry::load_from_dir_with_terrain("content/stages", &terrain_registry);
