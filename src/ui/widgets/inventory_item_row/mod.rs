//! Module Name: InventoryItemRow Widget — 物品行复合控件
//!
//! 组合 Panel / Text / Button 三个原子组件为一个水平物品行。
//! 每个行显示物品图标名称、数量和"使用"按钮。
//!
//! Contract:
//!   Props (input):    item_name, quantity（通过 InventoryItemRowState）
//!   Events (output):  InventoryItemAction::Use 标记在按钮实体上供 Observer 路由
//!   Local State:      InventoryItemRowState（item_name, quantity）
//!
