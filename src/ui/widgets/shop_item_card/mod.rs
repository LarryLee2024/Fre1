//! Module Name: ShopItemCard Widget — 商品卡片复合控件
//!
//! 组合 Panel / Text / Button 三个原子组件为一个商品卡片。
//! 每个卡片显示物品名称、价格、库存和购买按钮。
//!
//! Contract:
//!   Props (input):    item_name, price, stock（通过 ShopItemCard）
//!   Events (output):  ShopItemAction::Buy 标记在按钮实体上供 Observer 路由
//!   Local State:      ShopItemCard（item_id, price, stock）
//!
