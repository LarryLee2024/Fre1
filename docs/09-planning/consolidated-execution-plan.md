...
created: 2026-06-18
updated: 2026-06-19 (Phase E completed: Full Content extension — 4 remaining Def types)
tags:
  - planning
  - implementation
  - roadmap
...
| **横切层** | app / content / tools / modding | 11 | ~560 | ✅ 内容层完整实现（15 类型全部 Asset 化） |
...
| **基础设施接入规划** | **22 个未使用系统接入** | **`docs/09-planning/infrastructure-integration-plan.md`** | **🟡 规划完成，待执行** |
...
| **Phase E Content Extension** | **SummonTemplateDef/CampEventDef/BondDef/EnchantmentDef 全部接入 Content 加载管线（Asset derive + DefinitionType impl + ContentPlugin dispatch + Resources + RON 示例）** | **2026-06-19** | **✅ 已完成（1513 tests, 0 fail）** |
...
| TestDebt-002 | combat integration facade 测试补全（effect/ability/trigger/event） | 2026-06-20 | ✅ 已完成 |
| Spell Formulas 重构 | proficiency_bonus_for_level 委托 + concentration_dc 参数化 | 2026-06-20 | ✅ 已完成 |
...
| GAP-4 | **Content 加载未就绪** | ContentPlugin 现注册 15 种类型的 Asset 加载 + 校验 + 存储 | Phase E | ✅ 已完成：全量 Content 扩展（1513 tests） |
...
| 阶段 | 内容 | 前置条件 |
|------|------|---------|
| Phase E | Content 系统（配置加载 → 校验 → Registry 注册） | ✅ 已完成（15 种类型全部 Asset 化，端到端 RON 配置加载管线完成） |
