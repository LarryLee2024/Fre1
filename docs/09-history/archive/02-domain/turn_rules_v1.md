---
id: history.archive.turn_rules_v1
title: turn_rules_v1
status: archived
...
发现状态机 OnEnter/OnExit 包含业务逻辑时，必须停止。必须输出：
...
| 宪法条款 | 本领域对应 |
|----------|-----------|
| 1.1.4 逻辑与表现分离 | 状态机只管流程，不管表现 |
| 2.1.2 数据与行为分离 | OnEnter/OnExit 保持轻量 |
| 5.0 通信三原则 | TurnStarted/TurnEnded Message |
| 2.1.5 Resource 不是全局仓库 | TurnOrder/TurnState 是唯一真相源 |
...
| 违规行为 | 检测方式 | 输出 |
|----------|----------|------|
| 状态机 OnEnter/OnExit 包含业务逻辑 | 代码审查 | ARCHITECTURE VIOLATION: 状态机 OnEnter/OnExit 包含业务逻辑，违反"状态机只负责流程"原则。 |
