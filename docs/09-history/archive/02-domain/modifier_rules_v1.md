---
id: history.archive.modifier_rules_v1
title: modifier_rules_v1
status: archived
...
要求：
- 🟥 RON 配置路径：assets/rules/（宪法 1.1.5）
- 🟥 双标签同时匹配
...
原因：Calculator 通过 type_name() 分发（宪法 6.0.2）
...
原因：宪法 1.1.3 Rule/Content 分离，规则逻辑由代码实现，规则内容由 RON 配置
...
| 宪法条款 | 本领域对应 |
|----------|-----------|
| 1.1.3 Rule/Content 分离 | 修饰管线是规则，RON 配置是内容 |
| 1.1.5 数据驱动 | ModifierRule 从 RON 加载 |
| 6.0.2 Trait 用于扩展点 | ModifierCalculator trait 替代 match |
| 2.2.1 禁止直接修改最终属性 | 修饰通过管线链式计算 |
...
| 硬编码规则逻辑 | 代码审查 | ARCHITECTURE VIOLATION: 硬编码修饰规则，违反 Rule/Content 分离原则。 |
