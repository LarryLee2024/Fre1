---
id: history.archive.character_rules_v1
title: character_rules_v1
status: archived
...
输出：Attributes 组件
🟥 禁止：直接设置 Vital Resources 基础值（宪法 2.2.1）
...
原因：Entity 只是 ID，不承载行为（宪法 2.1.1）
...
原因：UnitTemplate 是共享配置，修改影响所有实例（宪法 1.1.2）
...
原因：死亡处理由 Dead Hook + Observer + Message 分层处理（宪法 5.0）
...
| 违规行为 | 检测方式 | 输出 |
|----------|----------|------|
| 运行时修改 UnitTemplate | 代码审查 | ARCHITECTURE VIOLATION: 运行时修改 UnitTemplate，违反 Definition/Instance 分离原则。 |
| 用 bool 代替 Tag Component | 代码审查 | ARCHITECTURE VIOLATION: 使用 is_dead: bool 代替 Dead Tag，违反宪法 2.1.4。 |
| Entity 承载行为 | 代码审查 | ARCHITECTURE VIOLATION: Entity 承载行为（如 player.attack()），违反"Entity 只是 ID"原则。 |
