---
id: history.archive.buff_rules_v1
title: buff_rules_v1
status: archived
owner: domain-designer
created: 2026-06-14
updated: 2026-06-14
superseded_by: ../../02-domain/buff/
---
...
要求：
- 🟥 不可变，加载后不修改（宪法 1.1.2）
- 🟥 RON 配置路径：assets/buffs/（宪法 1.1.5）
...
原因：修饰符是属性计算的一部分，不清理导致属性错误（宪法 2.2.1）
...
原因：DoT 伤害可导致角色死亡（宪法 2.1.4 Dead Tag）
...
违反后果：Buff 修饰未经管线，属性计算不一致

架构违规检测：

```
ARCHITECTURE VIOLATION: Buff 直接修改属性值，违反"属性修改必须通过 Modifier 管线"原则。
```
...
| 违规行为 | 检测方式 | 输出 |
|----------|----------|------|
| 过期 Buff 修饰符未清理 | 代码审查 | ARCHITECTURE VIOLATION: 过期 Buff [buff_id] 的修饰符未清理，违反"过期 Buff 修饰符必须清理"不变量。 |
| Buff 直接修改属性值 | 代码审查 | ARCHITECTURE VIOLATION: Buff 直接修改属性值，违反"属性修改必须通过 Modifier 管线"原则。 |
| 增量更新标签 | 代码审查 | ARCHITECTURE VIOLATION: 增量更新标签，违反"标签必须完全重建"原则。 |
