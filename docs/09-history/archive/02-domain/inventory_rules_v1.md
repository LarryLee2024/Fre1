---
id: history.archive.inventory_rules_v1
title: inventory_rules_v1
status: archived
owner: domain-designer
created: 2026-06-14
updated: 2026-06-14
superseded_by: ../../02-domain/inventory/
---
...
要求：
- 🟥 不可变，加载后不修改（宪法 1.1.2）
- 🟥 RON 配置路径：assets/items/（宪法 1.1.5）
...
```
ARCHITECTURE VIOLATION: 消耗品直接修改属性值，违反"属性修改必须通过 Modifier 管线"原则。
```
...
| 宪法条款 | 本领域对应 |
|----------|-----------|
| 1.1.2 Definition/Instance 分离 | ItemDef(Definition) vs ItemInstance(Instance) |
| 1.1.3 Rule/Content 分离 | 容器操作是规则，RON 配置是内容 |
| 1.1.5 数据驱动 | ItemDef 从 RON 加载 |
| 2.1.2 数据与行为分离 | Container 是数据容器 |
| 2.2.1 禁止直接修改最终属性 | RestoreVital 通过 Modifier 管线 |
| 5.0 通信三原则 | UseItem/TransferItem Message |
...
| 违规行为 | 检测方式 | 输出 |
|----------|----------|------|
| 消耗品直接修改属性值 | 代码审查 | ARCHITECTURE VIOLATION: 消耗品直接修改属性值，违反"属性修改必须通过 Modifier 管线"原则。 |
| 运行时修改 ItemDef | 代码审查 | ARCHITECTURE VIOLATION: 运行时修改 ItemDef，违反 Definition/Instance 分离原则。 |
