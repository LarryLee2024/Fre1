...
pub struct ScreenInfo {
    pub screen_type: ScreenType,      // 页面类型
    pub entity: Entity,               // 页面根实体
    pub transition: TransitionType,   // 过渡动画类型
}

#[derive(Clone, Reflect, PartialEq)]
pub enum ScreenType {
    MainMenu, Battle, Inventory, Shop,
    Settings, SaveLoad, QuestLog, CharacterDetail,
}
```
...
| 操作 | 行为 | 前置条件 | 后置条件 |
|------|------|---------|---------|
| push(screen) | 推入新 Screen 到栈顶 | 栈深度 < max_depth | 当前 Screen → Background，新 Screen → Active |
| pop() | 弹出栈顶 Screen | 栈非空 | 栈顶 Screen → Unloading → Destroyed，下层 Screen → Active |
| replace(screen) | 替换栈顶 Screen | 栈非空 | 栈顶 Screen → Unloading → Destroyed，新 Screen → Active（深度不变）|
