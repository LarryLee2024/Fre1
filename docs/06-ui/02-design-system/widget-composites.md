...
> **注意**: 当前实际目录结构为扁平布局（所有 Widget 在 `widgets/` 下平铺），而非 `widgets/composites/molecules/` 和 `widgets/composites/organisms/` 的子目录结构。两方案均可接受，但需要保持一致。建议未来重构时统一。
