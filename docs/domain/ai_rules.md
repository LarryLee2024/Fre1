...
核心原则：
- AI 通过 Intent + Effect Pipeline 表达和执行行动，与玩家共用同一通道
- 行为配置数据驱动（RON 文件），策略逻辑通过 Trait 注册表分发
- AI 决策不访问玩家不可见信息，不直接修改 ECS 组件
...
## 不变量1：AI 必须通过 Intent 表达意图

任意时刻：

AI 产生的任何行动必须通过 CombatIntent 或 MovementIntent 表达。AI 不得直接修改 ECS 组件。
...
## 不变量2：AI 与玩家共用 Effect Pipeline

任意时刻：

AI 的攻击执行必须经过 Generate → Modify → Execute 三步管线。AI 和玩家的伤害计算路径必须完全相同。
...
禁止：
- AI 直接修改 ECS 组件（如直接设置 `unit.acted = true`）
- AI 绕过 CombatIntent 独立执行攻击
...
禁止：
- 在 Rust 代码中硬编码 AI 行为配置
- 修改已有的 RON 配置文件格式
- 在运行时修改 BehaviorTemplate 的 Definition 数据
...
禁止：AI 直接修改 ECS 组件（如直接设置 unit.acted、unit.hp 等）
...
禁止：在 Rust 代码中硬编码 AI 行为配置（if-else 难度分支、硬编码策略选择等）
...
禁止：修改已有的 RON 配置文件格式而不更新 AiBehaviorDef
