...
核心原则：
- 回合阶段状态机驱动单位行动循环，NextState 驱动转换
...
- 持续效果结算（参见 `docs/domain/duration_rules.md`）：所有Buff的duration在此阶段递减
- 触发点（参见 `docs/domain/trigger_rules.md`）：TurnStart和TurnEnd是Buff的触发时机点
...
- 关卡地图数据加载（由 Map 领域负责）
- Buff的持续策略管理（由 `docs/domain/duration_rules.md` 负责）
- Buff的触发时机判断（由 `docs/domain/trigger_rules.md` 负责）
- Buff的叠层合并逻辑（由 `docs/domain/stack_policy_rules.md` 负责）
