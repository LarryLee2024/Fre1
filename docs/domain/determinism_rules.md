...
Version: 1.0
Status: Proposed
> **优化来源**: `docs/architecture/determinism_rules.md`（吸收 48.md 内容：预生成随机流、派生种子、LogicalId 排序、整数溢出防护、浮点污染隔离墙、ChaCha8Rng、tick 排序）
...
> **优化来源**: `docs/architecture/determinism_rules.md`
...
战斗 System 通过 `Res<RandomStream>`（只读锁）获取随机数，禁止通过 `ResMut<BattleRng>` 实时获取。RandomStream 在 UnitSpawn 阶段（战斗前）预生成，与 `docs/architecture/determinism_rules.md` 保持一致。
