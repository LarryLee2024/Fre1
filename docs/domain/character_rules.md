...
核心原则：
- 单位定义由 UnitTemplate 驱动，禁止代码硬编码
- Definition/Instance 分离：UnitTemplate 是不可变配置，Unit 是运行时实例
- 单位死亡标记 Dead 组件，禁止立即 despawn Entity
...
HP 降为 0 时添加的组件标记，触发 Hook 自动清理和 Observer 广播。
...
## 不变量1：单位创建必须通过 UnitTemplate

任意时刻：

所有战斗单位必须通过 UnitTemplate 生成。禁止代码中硬编码单位属性值（如直接设置 base_attributes）。
...
## 不变量4：Definition/Instance 分离

任意时刻：

UnitTemplate（定义态）不可变，不可被运行时修改。所有运行时变化通过修饰器管线作用于 Unit 实例的 Attributes。
