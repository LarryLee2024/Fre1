...
核心原则：
- 所有属性修改必须通过修饰器栈，禁止直接修改
- Buff、装备、特质统一通过修饰器机制修改属性（统一入口）
- 效果管线 Generate → Modify → Execute 三步严格顺序执行
- 修饰器链（Modifier Chain）是Effect执行链路中的修饰环节：暴击→属性克制→地形加成→Buff加成→最终结果
...
从核心属性实时计算的属性，不存储 base 值。

不是核心属性。不是生命资源。不是可缓存的值。

关键属性：
- 实时计算，每次 get() 调用重新求值
- 公式固定（如 Attack = Might × 2，Defense = Vitality）
- 可被修饰器叠加（Add / Multiply）
- 不可通过 set_base() 设置（set_base 对 Derived Stat 无效）

---

## 修饰器（Modifier）

对某个属性的增量或倍率修改，携带来源标识和操作类型。
...
一个动作（技能施放、攻击等）产生的待处理结果，如伤害、治疗、施加 Buff。
...
关键属性：
- GenerateContext：source_entity、target_entity、source_attrs、target_attrs、defense_bonus、skill_id、source_tags、terrain_id
- PreviewContext：source_attrs、target_attrs、terrain_defense_bonus、buff_registry
- ExecuteContext：持有 &mut World，提供 apply_damage / apply_heal / apply_buff / apply_cleanse 方法
- 纯数据传递，不存储持久状态
...
## 标签（GameplayTag / TagName）

参见 `shared_layer_rules.md#标签`。

本领域使用位掩码标签进行修饰规则匹配、Buff 分类、技能类型判定。标签的类型定义、位分配、TagName↔GameplayTag 映射由 Shared Layer 领域负责。

---

## 特质（Trait）

角色内在的能力修改器，修改属性和提供被动效果。
...
- 触发时机：OnAttack / OnHit / OnKill
- 通过 TraitEffect → Effect Pipeline 执行
- TraitSource 标识来源（Race / Class / Equipment / Buff / Talent）

---

## 效果处理器（EffectHandler）

描述如何生成、预览、执行一种效果类型的 trait 实现。

不是修饰规则。不是属性修饰器。

关键属性：
- 每种效果类型实现一个 EffectHandler（DamageHandler / HealHandler / BuffHandler / CleanseHandler）
- type_name 方法返回效果类型名（与 EffectDef::type_name 对应）
- 通过 EffectHandlerRegistry 注册，运行时 HashMap 查找分发
...
对属性的所有增量修改（Buff/装备/特质）必须通过 add_modifier / remove_modifiers_from 接口，禁止直接修改 base 值或 current_hp（生命资源的 set_vital 除外）。
...
set_base() 仅对 Core Stat 有效，对 Derived Stat 和 Vital Resource 调用 set_base 时应产生警告日志且不修改值。
...
战斗效果必须经过 Generate → Modify → Execute 三步管线，禁止跳步执行。
...
属性最终值 = (base + sum(所有 Add 修饰器值)) × product(所有 Multiply 修饰器值)。Multiply 修饰器无 Add 时默认乘数为 1.0（不是 0）。
