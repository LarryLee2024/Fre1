...
核心原则：
- 技能是 Rule，火球术是 Content（Rule/Content 分离）
- 技能系统只负责验证和路由，不执行效果
- 技能释放前必须通过验证管线
...
每次技能释放前，必须调用 can_use() 验证所有条件（冷却 / 消耗 / 标签 / HP 阈值）。验证失败时返回 SkillUseError，不执行效果。
...
## 不变量4：技能效果通过 Effect Pipeline 执行

任意时刻：

技能的效果列表（SkillData.effects）中的每个 EffectDef 必须通过 Effect Pipeline 的 Generate → Modify → Execute 三步执行，禁止跳步或直接执行。
