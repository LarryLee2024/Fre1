...
核心原则：
- Effect 只负责指定 FormulaId，不负责计算公式
- Formula 计算是纯函数：相同输入 → 相同输出
- Formula 不包含随机逻辑（随机由独立的 Random 系统处理）
...
Formula.calculate(input) 必须是纯函数：相同 FormulaInput 产生相同 FormulaOutput，无副作用。
...
## 不变量2：Formula 不包含随机逻辑

任意时刻：

Formula 计算不包含随机数生成。随机性由独立的 Random 系统注入到 FormulaInput 中。
...
| EffectHandler trait 分发 | `docs/domain/attribute_modifier_rules.md#效果处理器` |
| Modifier 修饰链 | `docs/domain/attribute_modifier_rules.md#修饰规则` |
| Buff 叠层策略 | `docs/domain/stack_policy_rules.md` |
| 触发器和上下文 | `docs/domain/trigger_rules.md` |
| 技能定义和验证 | `docs/domain/skill_rules.md` |
