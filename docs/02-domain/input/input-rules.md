---
id: 02-domain.input.input-rules
title: Input Rules
status: draft
...
核心原则：
- 🟩 输入系统只负责采集和路由，不执行游戏逻辑（宪法 11.6.1 输入为独立 Feature）
- 🟩 所有用户操作通过 UiCommand Message 传递（宪法 11.5.1 命令层标准化）
- 🟩 AI 回合期间玩家输入完全忽略
- 🟥 禁止业务模块直接读取原始输入按键（宪法 11.6.1）
- 🟩 输入转换为标准化业务命令后，执行不区分来源（宪法 11.5.2 命令无差别执行）
...
| 条款编号 | 条款名称 | 合规状态 | 说明 |
|----------|----------|----------|------|
| 11.6.1 | 输入为独立 Feature | 🟩 已合规 | 输入在 input 模块统一处理，业务模块不读原始输入 |
| 11.5.1 | 所有操作入口为命令 | 🟩 已合规 | 用户操作转换为 UiCommand Message |
| 11.5.2 | 命令无差别执行 | 🟩 已合规 | command_handler 不区分命令来源 |
| 1.3.1 | 三层架构划分 | 🟩 已合规 | 输入属于表现层，通过 Message 传递到应用层 |
| 1.3.2 | 依赖方向铁则 | 🟩 已合规 | Input → Core（单向依赖） |
| 2.2.4 | Message 跨域广播 | 🟩 已合规 | UiCommand 作为 Message 传递到 command_handler |
| 2.3.8 | Schedule 权责划分 | 🟩 已合规 | 输入处理在 PreUpdate 阶段执行 |
| 11.7.1 | 读路径无副作用 | 🟩 已合规 | 输入采集为纯读操作 |
| 11.7.2 | 写路径收口 | 🟩 已合规 | 状态修改通过 UiCommand 统一处理 |
...
| 通信层级 | 用途 | 输入领域应用 |
|----------|------|-------------|
| Hook（2.2.1） | 组件生命周期 | 无（输入不操作组件生命周期） |
| Trigger（2.2.2） | Feature 内事件链 | 无（输入不产生事件链） |
| Observer（2.2.3） | 局部状态变化响应 | HoveredEntity 变化触发 ViewModel 更新 |
| Message（2.2.4） | 跨域广播 | UiCommand Message 传递到 command_handler |

禁止事项（宪法 2.2.5）：
- 🟥 禁止在 Input 系统中直接修改 ECS 组件（必须通过 UiCommand）
- 🟥 禁止业务模块直接读取原始输入（宪法 11.6.1）
...
以下禁止事项源自 AI 开发宪法，输入领域必须严格遵守：

## 禁止：业务模块直接读取原始输入（宪法 11.6.1）

原因：输入为独立 Feature，业务模块必须通过 UiCommand 获取用户意图，禁止直接读取 Pointer/ButtonInput。

违反后果：业务逻辑与输入设备耦合、无法适配多端输入。

---

## 禁止：在 Input 系统中直接修改 ECS 组件（宪法 2.2.5）
...
## 禁止：输入路由产生副作用（宪法 11.7.1）

原因：输入采集和路由为纯读操作，不修改游戏状态。

违反后果：输入处理改变游戏状态、仿真失败。

---

## 禁止：跳过命令层直接执行业务逻辑（宪法 11.5.1）

原因：所有操作入口必须为标准化命令，输入系统只负责将原始输入转换为 UiCommand。

违反后果：业务逻辑与输入设备耦合、无法支持回放和 AI 复用。
...
## 禁止：为未来需求过度设计输入系统（宪法 1.1.7）

原因：当前输入抽象层已覆盖鼠标/键盘场景，手柄支持作为轻量扩展点预留，禁止提前实现完整手柄支持。
