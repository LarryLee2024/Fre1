...
| 术语 | 定义 | 职责边界 |
|------|------|----------|
| Quest | 任务，有目标/进度的可完成游戏内容单元 | 负责：任务的生命周期管理；包含 name_key: LocalizationKey（任务名称）、desc_key: LocalizationKey（任务描述）；不负责：任务的触发条件 |
| QuestState | 任务状态，定义任务所处的生命周期阶段 | 负责：状态的流转规则；不负责：状态变化后的行为 |
| Objective | 任务目标，任务的子目标/里程碑，需完成才能推进任务 | 负责：目标的条件定义与进度追踪；包含 description_key: LocalizationKey（目标描述）；不负责：目标完成后的奖励 |
...
- 🟥 禁止：任务/目标定义中直接存储自然语言文本 — 理由：任务名称、描述、目标描述必须通过 LocalizationKey 引用
...
- ✅ LocalizationKey：任务名称、描述、目标描述全部使用 LocalizationKey（宪法 §22）
