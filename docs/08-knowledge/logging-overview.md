...
owner: architect
created: 2026-06-19
updated: 2026-07-01（吸收 Domain/record 演进，消除 LogCode::target）
tags:
  - knowledge
  - logging
  - observability
...
│  │      emit_info!(LogCode::PRG002, entity = ?e.entity, old = e.old_level, │    │
│  │          new = e.new_level, "角色升级");                               │    │
│  │  }                                                           │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  额外输出后端：                                                       │
...
目前定义了 20 个域名、约 150 个编码。每个 LogCode 有三个核心方法：
...
其中 `event_name()` 是新增方法，让 LogCode 成为**单一事实源**：
- `event` 字符串从 LogCode 派生，不需要在 Observer 中手动写 `event = "level_up"`
- `target` 路由职责移至 `Domain::target()`（`shared/diagnostics/domain.rs`），LogCode 不再承担路由职责
...
2. **`emit_info!`/`emit_warn!`/`emit_debug!`** — 统一可观测性入口（宏内部自动完成 `telemetry::record(LogCode)` + `tracing::info!()`）

> 注意：emit 宏内部不指定 target，因为 target 继承自 `#[instrument]` span。
...
**全局计数器**：使用 `LazyLock<Mutex<HashMap<LogCode, u64>>>`，Observer 中通过 `emit_info!(LogCode::XXX, ...)` 的宏内部自动调用，无需 World 访问。
...
─────────────────────────
  progression_logger.rs:
    ...
    fn on_level_up(trigger: On<LevelUp>) {
        telemetry::record(LogCode::PRG002);           // ← 统一入口（metrics + trace）
        let e = trigger.event();
        info!(                                      // ← 只放变量
...
─────────────────────────
  ...
  │
  └─ emit_info! → telemetry::record(PRG002) → MetricsCollector
        ↓
      每 60 帧聚合输出:
...
pub(crate) fn on_item_crafted(trigger: On<ItemCrafted>) {
    emit_info!(LogCode::CRF003, crafter = ?event.crafter, item_id = ?event.item_id, "物品铸造完成");
}
```
...
- [ ] Observer 函数调用 `emit_info!(LogCode::XXX, ...)`（统一入口，而非直接调用 metrics::record）
...
| 旧版 Observer 升级 | ✅ 已完成 | 全部 20 个 observer 已统一模式：`#[instrument]` + `emit_info!` |
...
| **Observability Facade** | ✅ 已实现 | `telemetry::record(LogCode)` 已创建为 metrics::record 的封装 + `record_event(&event)` 预留 L2 入口。所有 Observer 通过 `emit_info!` 宏统一调用 |
...
| 3 | **Domain 路由分离** | `ObservableEvent::DOMAIN` 决定 tracing target，LogCode 不再承担路由职责 | 新增 `Domain` 枚举（`shared/diagnostics/domain.rs`），`Domain::target()` 导出「路由字符串」 |
...
| 6 | **`telemetry::record` + `record_event`（L2 预留）** | `emit_info!` 宏内部自动完成 `telemetry::record(LogCode)` + `tracing::info!()`，Observer 不再直接调用 `metrics::record` | 已实现基础版，`record_event` 为未来 L2 统一分发预留 |
...
Observer 从原来的三要素模式：
```
#[instrument] + telemetry::record(LogCode) + info!()
```
...
emit_info! 宏内部自动完成 `telemetry::record(LogCode)` + `tracing::info!()` + 结构化字段输出。
