...
| 测试 | 测试需要确定性的随机数，独立于游戏逻辑 | `DeterministicRng`（StdRng，shared/testing） |
...
`shared/random/mod.rs:97` 有一个标记为 `TODO[P2]` 的注释：

```rust
// TODO[P2][SHARED][2026-06-20]: rand 0.10 API 变更...
...
这是 `rand` crate 从 0.9 升级到 0.10 的 API 变更导致的。核心功能（`gen_range`、`gen_bool` 等通过 `rand::RngExt`）仍然正常，但 `RngCore` 和 `CryptoRng` trait 实现被注释掉了。这不影响当前功能，但意味着任何需要泛型 `Rng` trait 的代码不能直接传入 `SeededRng`。
...
## 5. 第三套系统：针对测试的 DeterministicRng（StdRng）
...
| 维度 | 回放版（replay/foundation） | 测试版（shared/testing） |
|------|---------------------------|------------------------|
...
**名字冲突：** 两者都叫 `DeterministicRng`，但它们是**完全不同的类型**，位于不同的 Rust 模块中。在代码中通过完整路径区分：

- `core::capabilities::runtime::replay::foundation::DeterministicRng`（生产）
- `shared::testing::DeterministicRng`（测试）

测试版的存在是为了让单元测试不需要依赖回放系统的完整基础设施。测试一个函数需要随机数时，直接用测试版 `DeterministicRng` 即可。
...
| 单元测试 | DeterministicRng（测试版） | ✅ 测试版 | 可用 |
...
| `DeterministicRng` 测试版（shared/testing/） | ✅ 全部实现（52 行） | ❌ 无专门测试 | ✅ 被测试代码直接使用 | 🟢 已实现、可使用 |
...
**一句话总结：** `DeterministicRng`（四流 MurmurHash3）是目标，`GameRng` / `SeededRng`（单流 ChaCha12）是过渡，`DeterministicRng`（测试版 StdRng）仅供测试。所有代码最终都应该通过 `ResMut<DeterministicRng>` 获取随机数，并指定正确的 `RngStream`。
...
| 测试版 DeterministicRng | `src/shared/testing/deterministic.rs` |
