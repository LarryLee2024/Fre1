...
![一个测试场景的橙色图像，带有曝光、色温、色调和色相控制。高光、中间调和阴影的饱和度、对比度、gamma、增益和 lift 均可分别配置。](https://bevy.org/news/bevy-0-14/filmic_color_grading.jpg)
...
fn on_damage_taken(
    ...
    // Observer 可以通过使用 commands 发送更多触发器来链式调用。
    // 这就是 observer 如此强大的原因……这个事件链会在第一个事件被触发时作为单个事务求值。
...
![基础着色器节点设置](https://bevy.org/news/bevy-0-14/basic_nodes.jpg)

结果是在 Bevy 中的以下场景：

![纹理被拉伸的场景](https://bevy.org/news/bevy-0-14/bevy_no_rep.jpg)

哦不！一切都被拉伸了！这是因为我们设置的 UV 将纹理恰好映射到网格上一次。有几种方法可以处理这个问题，但最方便的是添加缩放纹理的着色器节点，使其重复：

![重复纹理的着色器节点设置](https://bevy.org/news/bevy-0-14/rep_nodes.jpg)

`Mapping` 节点的数据就是导出到 `KHR_texture_transform` 的数据。看红色部分。这些缩放因子决定了纹理在材质中应重复多少次。调整所有纹理的这个值可以得到更好的渲染效果：

![纹理重复后的场景](https://bevy.org/news/bevy-0-14/bevy_rep.jpg)
...
![圆角边框示例截图](https://bevy.org/news/bevy-0-14/rounded_borders.jpg)
...
![调用 Transform::align 之前](https://bevy.org/news/bevy-0-14/align-before-move.jpg)
...
`align` 会移动它以尽可能匹配所需的位置：

![调用 Transform::align 之后](https://bevy.org/news/bevy-0-14/align-after-move.jpg)
