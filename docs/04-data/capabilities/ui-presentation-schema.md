...
- 当前仅包含三个核心 ViewModel；其余面板（InventoryVm、ShopVm、QuestLogVm、NotificationVm、ModalVm）将在后续迭代中按需加入 UiStore
...
pub struct BattleHudVm {
    ...
    /// 当前回合数
    pub turn_number: u32,
    /// 阶段描述（本地化 Key，如 "ui.phase.player_turn"）
    pub phase_key: &'static str,
}
```

**未来扩展**：以下字段尚未在代码中实现，规划在后续迭代中加入：

| 字段 | 类型 | 说明 |
|------|------|------|
| `active_character` | `Option<CharacterId>` | 当前行动角色 |
| `phase` | `BattlePhaseVm` | 战斗阶段枚举（替代 `phase_key` 字符串） |
| `cooldowns` | `HashMap<SkillId, u32>` | 技能冷却映射 |

```rust
/// 战斗阶段视图（规划中，当前使用 phase_key 字符串替代）
...
|------|------|------|
| `exp` | `u32` | 当前经验值 |
| `exp_to_next` | `u32` | 升级所需经验 |
| `buffs` | `Vec<BuffVm>` | Buff/Debuff 列表 |
| `status_effects` | `Vec<StatusEffectVm>` | 状态效果列表 |
| `stats` | `StatsVm` | 属性面板（攻击、防御、速度等） |
...
| 字段 | 类型 | 所属类型 | 说明 |
|------|------|---------|------|
| `selected` | `Option<SkillId>` | SkillPanelVm | 当前选中技能 |
| `ap_remaining` | `u32` | SkillPanelVm | 剩余行动点 |
| `max_ap` | `u32` | SkillPanelVm | 最大行动点 |
| `desc_key` | `UiTextKey` | SkillSlotVm | 技能描述本地化 Key |
| `icon_key` | `AssetKey` | SkillSlotVm | 技能图标资源 Key |
| `mp_cost` | `u32` | SkillSlotVm | MP 消耗 |
...
当前已实现的 ViewModel 和核心类型必须在 UiPlugin 中注册：
...
**未来类型注册**：以下类型为规划中的 ViewModel/枚举，在代码实现后需补充注册：

```rust
// 规划中的 ViewModel 注册（实现后取消注释）
