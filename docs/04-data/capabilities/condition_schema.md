...
| 数据类别 | 归属层 | 说明 |
|----------|--------|------|
| `Condition` | Definition | 统一条件定义（TagRequirement / TagMatch / AttributeCheck / ResourceCheck / Custom） |
...
- TagRequirement（单标签）、TagMatch（多标签+层级）、AttributeCheck、ResourceCheck 四种内置类型的数据表达
- 免疫条件的最高优先级处理
...
enum Condition {
    ...
    /// 多标签匹配——基于 TagQuery 的 Any/All/None + 层级继承检查
    TagMatch(TagQuery),
...
| `TagRequirement` | Definition | 是（Def 内嵌） | 是 | 归属 Tag 领域（单标签检查） |
| `TagMatch` | Definition | 是（Def 内嵌） | 是 | 归属 Tag 领域（多标签 + 层级匹配） |
...
| 依赖方向 | 依赖 Schema | 说明 |
|----------|------------|------|
| 依赖 | → TagSchema | TagRequirement.target_tags 引用 TagId；TagMatch.query 引用 TagQuery |
