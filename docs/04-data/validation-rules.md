...
| 条款 | 级别 | 落地规则 |
|------|------|----------|
| 12.2.1 配置必须有 Schema | 🟥 | 所有配置结构体必须对应明确的 Schema 定义 |
| 12.2.2 CI 配置 Lint | 🟥 | 所有配置必须支持 CI 自动校验：引用合法性、数值范围、循环依赖 |
| 12.1.6 自动校验 | 🟥 | 配置之间的引用关系必须实现自动校验 |
| 20.1.1 质量预算 | 🟥 | Warning Budget = 0，Bug Budget = 0，Tech Debt Budget = 可控 |
| 20.7 CI 门禁 | 🟥 | 配置数据校验全部通过才能合并入主分支 |
...
- 🟥 CI 中执行配置 Lint 校验（12.2.2）：引用合法性、数值范围、循环依赖
...
| 检查点 | 校验范围 |
|--------|----------|
| 回合结束（TurnEnd） | 所有存活单位 HP/MP/Stamina 范围、Buff 数量不超限、Modifier 来源完整、技能冷却合法 |
| 战斗结束（BattleEnd） | 全局不变量（胜负不共存、回合队列状态、阵营一致性） |
| 状态转换（StateTransition） | 状态机转换路径合法性、转换前后数据一致性 |
| 关卡加载后（LevelLoad） | 配置数据合法性（MaxHP 正值、属性范围、地图边界） |
| 存档加载后（SaveLoad） | 数据完整性（版本号、时间戳、ID 引用、属性范围、Buff 数量、单位位置） |
| MOD 加载后（ModLoad） | MOD 数据合法性（ID 唯一性、引用完整性、权限范围、数值合法性、格式合规） |

> **优化来源**: docs/01-architecture/validation_rules.md
...
## 规则6：校验性能预算 [宪法 20.1.1 🟥]

🟥 **质量预算原则**：Warning Budget = 0，Bug Budget = 0，Tech Debt Budget = 可控。

必须：
- 🟥 单帧内全部校验逻辑总耗时不超过 2ms
- 🟥 关键不变量校验（伤害非负、HP 范围）在任何构建中都必须执行
- 🟥 非关键校验在 Release 构建中可选择性关闭以满足性能预算
- 🟥 主分支不允许存在未处理的编译警告（Warning Budget = 0）
- 🟥 P0/P1 级缺陷不允许流入主分支（Bug Budget = 0）
- 🟥 技术债必须登记跟踪，设定偿还节点，禁止无限制积累

禁止：
- 校验逻辑占用超过 2ms 帧预算
- 以性能为由跳过关键不变量校验（damage ≥ 0、HP 范围）
- 🟥 未处理的编译警告进入主分支
- 🟥 P0/P1 级缺陷进入主分支

允许：
- 分帧校验：将大量校验分散到多帧执行（不超过 2ms/帧）
- 按模块部分校验：仅校验本帧发生变更的模块
- 惰性校验：仅在访问时校验（延迟到实际使用时）
...
| 层级 | 校验职责 | 禁止 |
|------|---------|------|
| Core 层 | 领域不变量（HP 范围、伤害非负、Buff 上限） | 校验 UI 数据一致性 |
| Infrastructure 层 | 数据持久化完整性（存档校验和、文件格式） | 校验业务规则 |
| Content 层 | 配置数据合法性（引用完整性、数值范围） | 校验运行时状态 |
| UI 层 | 数据展示一致性（ViewModel 与 Core 状态同步） | 校验业务逻辑 |
| MOD 层 | MOD 数据安全（权限、引用、格式） | 校验内置数据 |

约束：
- 🟥 每层只校验本层职责范围内的不变量
- 🟥 跨层校验违规时记录 WARN 日志并上报，不直接修正
- 🟩 Core 层校验失败时触发 Panic（数据损坏不可恢复）
- 🟩 Content 层校验失败时触发 Reject（阻止非法配置注册）
