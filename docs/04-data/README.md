...
> **版本**: 1.1 | **角色**: @data-architect | **宪法优先级**: 🟩 必须遵守
...
| MapDef | `map:` | `map:dragon_peak` | String ID（语义化名称） |
| RecipeDef | `rcp_` | `rcp_000001` | 000000–999999 |
| LootTableDef | `ltb_` | `ltb_000001` | 000000–999999 |
| BondDef | `bnd_` | `bnd_000001` | 000000–999999 |
| CampEvent | `cmp_` | `cmp_000001` | 000000–999999 |
| Character | `char_` | `char_000001` | 000000–999999 |
| Class | `cls_` | `cls_000001` | 000000–999999 |
| Equipment | `equip_` | `equip_000001` | 000000–999999 |
| FormationDef | `fmd_` | `fmd_000001` | 000000–999999 |
| Subclass | `sub_` | `sub_000001` | 000000–999999 |
| Talent | `tal_` | `tal_000001` | 000000–999999 |
| Team | `team_` | `team_000001` | 000000–999999 |
| Unit | `unit_` | `unit_000001` | 000000–999999 |

**规则：**
- 禁止语义化 ID（❌ `ability.fireball` → ✅ `abl_000042`）
- **例外**：MapDef 使用语义化名称（`map:dragon_peak`），原因是 MapDef 是 L4 World 层数据，通过 AssetServer 路径加载而非 DefRegistry 注册，且地图文件本身由 Importer 从 TMX 自动生成，命名由关卡设计师控制
...
> **注：** 此表为 Definition ID 前缀总览。代码中完整的 ID 类型清单见 `src/shared/ids/types.rs`。
> 重构计划见 `docs/11-refactor/id-system-refactoring-2026-06-20.md`。

### 3.2 本地化 Key 格式

所有用户可见文本字段在 Definition 中只存储 LocalizationKey，采用 4 段式格式：
...
- [ ] Replay 兼容性是否满足？
- [ ] 四层分配是否清晰无跨层污染？
- [ ] 用户可见文本是否全部使用 LocalizationKey？
- [ ] LocalizationKey 是否以 en-US 为标准模板？
...
| 013 | **用户可见文本必须使用 LocalizationKey** | 代码中禁止出现用户可见自然语言文本；所有 UI、剧情、错误消息必须通过 Key 引用 | 编译期 Key 校验失败 |
| 014 | **LocalizationKey 以 en-US 为标准模板** | 所有 Key 必须在 en-US 的 .ftl 中有定义；其他 locale 缺失 Key 使用 en-US 回退 | 启动时 panic |
...
│   ├── id_strategy.md               # ID 策略详述（格式、分配、生命周期）
│   ├── id-taxonomy.md               # ID 分类体系（五类 ID 的职责边界）
...
| `foundation/id-taxonomy.md` | ✅ stable | data-architect | 2026-06-20 |
...
| `foundation/migration_policy.md` | ✅ stable | data-architect | 2026-06-21 |
...
| `capabilities/status_category_schema.md` | ✅ stable | data-architect | 2026-06-20 |
| `capabilities/ui-presentation-schema.md` | 🟡 draft | data-architect | 2026-06-19 |
...
| `infrastructure/logging_schema.md` | ✅ stable | data-architect | 2026-06-21 |
| `infrastructure/localization_schema.md` | ✅ accepted | data-architect | 2026-06-19 |
| `infrastructure/camera_schema.md` | 🟡 draft | data-architect | 2026-06-21 |
| `infrastructure/map-asset-schema.md` | 🟡 draft | data-architect | 2026-06-22 |
| `infrastructure/map-importer-schema.md` | 🟡 draft | data-architect | 2026-06-22 |
| `domains/README.md` | ✅ stable | data-architect | 2026-06-16 |
| `domains/tactical_schema.md` | ✅ stable | data-architect | 2026-06-16 |
| `domains/terrain_schema.md` | ✅ stable | data-architect | 2026-06-16 |
| `domains/element_schema.md` | ✅ stable | data-architect | 2026-06-20 |
