...
**设计原则**：LogCode 只负责事件编码，不负责路由追踪。
`target` 路由职责已移至 `Domain` 枚举（`shared/diagnostics/domain.rs`），
`ObservableEvent::DOMAIN` 决定 tracing target。
不再使用 `LogCode::target()`。

### 3.1.2 Domain 枚举（路由职责）

```rust
/// 可观测域——决定 tracing target 的路由标识。
/// LogCode 的编码职责与 Domain 的路由职责彻底分离。
enum Domain {
    Combat,    // target: "domain.combat"
    Tactical,  // target: "domain.tactical"
    Ability,   // target: "domain.ability"
    // ... 共 24 个变体
}
...
- **`telemetry::record` 扩展**：已实现基础的 `record(LogCode)` 和 `record_event(&event)`。`record_event` 未来可扩展为统一分发到 LoggerSink / MetricSink / ReplaySink / AuditSink。
