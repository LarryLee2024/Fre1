...
> **当前状态**: 以下 `IdAllocator` 设计为架构目标。当前 String ID 通过 `define_string_id!` 宏直接构造（`types/string_ids.rs`），运行时实例 ID 通过 `RuntimeIdAllocator`（`types/runtime_id.rs`）分配带 generation 保护的 ID。集中式 `IdAllocator` 为预留设计，待 Registry 系统成熟后接入。
...
> **当前状态**: 以下校验流程为架构目标。当前 `define_string_id!` 已实现 `checked_new()` 方法用于格式校验（检查非空、无冒号、仅字母数字下划线）。自动分配器 `IdAllocator` 待实现。
>
> 格式校验详情见 `docs/11-refactor/id-system-refactoring-2026-06-20.md §5`。
...
| `spell_schema.md` | `spl_` | Spell ID 被 Spell/Ability 引用 |
| `buff_schema.md` | `buf_` | Buff ID 被 Effect/Condition 引用 |
| `terrain_schema.md` | `ter_` | Terrain ID 被 Tactical/Terrain 引用 |
| `trigger_schema.md` | `trg_` | Trigger ID 被 Effect/Condition 引用 |
| `cue_schema.md` | `cue_` | Cue ID 被 UI/叙事 引用 |
| `recipe_schema.md` | `rcp_` | Recipe ID 被 Crafting 引用 |
| `loot_schema.md` | `ltb_` | LootTable ID 被 Loot/Quest 引用 |
| `character_schema.md` | `char_` | Character ID 被 Party/Inventory 引用 |
| `unit_schema.md` | `unit_` | Unit ID 被 Combat/Tactical 引用 |
| `equipment_schema.md` | `equip_` | Equipment ID 被 Inventory/Crafting 引用 |
| `team_schema.md` | `team_` | Team ID 被 Combat/Party 引用 |
| `class_schema.md` | `cls_` | Class ID 被 Progression/Party 引用 |
| `talent_schema.md` | `tal_` | Talent ID 被 Progression 引用 |
| `subclass_schema.md` | `sub_` | Subclass ID 被 Progression 引用 |
| `bond_schema.md` | `bnd_` | BondDef ID 被 Party 引用 |
| `formation_schema.md` | `fmd_` | FormationDef ID 被 Combat/Tactical 引用 |
| `camp_event_schema.md` | `cmp_` | CampEvent ID 被 CampRest 引用 |
| `localization_schema.md` | —（无独立 ID 前缀） | LocalizationKey 复用各领域的 Definition ID，在前者基础上拼接 namespace 和 suffix |

> **注**: 此表列出所有已注册的 Definition ID 前缀。完整类型定义见 `src/shared/ids/types/string_ids.rs`。

---

## 9. ID 分类体系

> **深度展开**: 见 `docs/04-data/foundation/id-taxonomy.md`

ID 按生命周期和职责分为五类：

| 类别 | 来源 | 存储 | 特点 |
|------|------|------|------|
| **Template ID** | 配置文件 | String/SmolStr | 永久稳定、Mod 友好 |
| **Runtime ID** | IdAllocator | index + generation | 每局唯一、可回收 |
| **Save ID** | 首次实例化 | Uuid | 跨存档、跨版本 |
| **Entity** | Bevy ECS | Entity | 零成本、仅基础设施层可见 |
| **Network ID** | 网络会话协商 | u32 | 确定性、低带宽 |

**硬规则**: Domain 层禁止裸 Entity/u64 作为业务标识。
