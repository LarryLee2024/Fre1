...
Buff/Debuff 系统是 SRPG 战斗的核心机制之一。当前项目已有 8 个 Buff（attack_up、attack_down、defense_up、defense_down、burn、poison、regen、stun）通过 RON 配置驱动，但缺少正式的数据模型架构决策。现有 `BuffData` 结构为扁平设计，缺少 Trigger、Duration 策略、Stack 策略等核心抽象，与 `docs/01-architecture/01-battle-gas/skill-buff-abstraction.md` §2.4 定义的 Buff 抽象模型存在差距。
...
- `docs/01-architecture/01-battle-gas/skill-buff-abstraction.md` — §2.4 Buff = Trigger[] + Effect[] 模型、§2.2 Skill = Selector + Effect[]、§4.6 Duration 策略、§4.7 StackPolicy 叠层系统、§4.8 Trigger 系统
