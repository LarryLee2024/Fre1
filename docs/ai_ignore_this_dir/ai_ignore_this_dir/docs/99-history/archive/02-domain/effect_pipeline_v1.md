---
id: history.archive.effect_pipeline_v1
title: effect_pipeline_v1
status: archived
owner: domain-designer
created: 2026-06-14
updated: 2026-06-14
superseded_by: ../../02-domain/attribute-modifier/attribute-modifier-rules.md
---
...
原因：管线保证 Generate → Modify → Execute 严格顺序（宪法 2.3.1）
...
原因：Generate 只生成效果，不执行效果（宪法 2.3.1）
...
原因：Modify 只修饰数值，不产生副作用（宪法 1.1.4）
...
原因：Handler 通过 type_name() 分发（宪法 6.0.2）
...
| 1.1.4 Logic/Presentation 分离 | 管线不包含 UI/动画逻辑 |
| 1.1.5 数据驱动 | EffectDef 从 RON 加载 |
| 1.1.9 测试优先 | Battle Replay + 自动化测试优先 |
| 2.1.1 Entity 只是 ID | source/target Entity 仅作 ID |
| 2.1.2 数据与行为分离 | GenerateContext 纯数据快照 |
| 2.1.5 Resource 不是全局仓库 | EffectQueue 有明确职责 |
| 2.2.1 禁止直接修改最终属性 | 伤害/治疗通过 Modifier 管线修饰 |
