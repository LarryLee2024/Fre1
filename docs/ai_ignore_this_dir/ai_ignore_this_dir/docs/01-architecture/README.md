...
> 本文件是架构规范的最高优先级文档。`docs/01-architecture/` 目录下 38 个设计文档按 10 个子目录分组，以下索引按类别列出所有文件及其核心内容摘要。

## 00-overview — 架构总纲（7 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `app-bootstrap.md` | App 层装配器设计：AppState 状态机、启动/关闭序列、SystemSet 排序、Schedule 组织 | 提议 |
| `layer-architecture-rules.md` | 七层架构分层组织原则、依赖规则和归属判定标准 | 提议 |
| `layer-contracts.md` | 七层边界定义、职责、依赖规则、禁止事项和三问判断法 | 提议 |
| `project-structure.md` | 项目完整目录结构规范（源码/资产/内容三棵树） | 提议 |
| `plugin-design.md` | Plugin 组织设计，声明式哲学，每个 Plugin 封装连贯功能片段 | 提议 |
| `plugin-contract-rules.md` | Plugin 间依赖规则、通信边界、初始化顺序和禁止事项 | 提议 |
| `schedules-design.md` | 自定义 Schedule、SystemSet 排序、状态门控调度、并行 vs 顺序策略 | 提议 |

## 01-battle-gas — 战斗/GAS（4 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `battle-fsm-design.md` | 战斗 FSM（PreBattle→RoundStart→PlayerPhase→EnemyPhase→TurnEnd→VictoryCheck→RoundEnd→PostBattle）、Guard/Action/Effect 三段式 | 提议 |
| `skill-buff-abstraction.md` | Effect Executor 统一抽象（500 技能收敛为 20-30 个 Executor）、SRPG Lite-GAS 10 模块对齐、ExecutionStack、TriggerRegistry | 提议 |
| `gas-architecture.md` | SRPG-GAS 系统架构设计：Trait Registry + 纯函数管线模式、14 子领域模块划分 | 提议 |
| `command-bus-design.md` | GameCommand 可序列化枚举、Memento 模式撤销、Cursor-based 执行、校验/执行分离 | 提议 |

## 02-ecs-patterns — 组件/ECS 模式（3 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `component-design-rules.md` | 四位一体组件分类（Marker/Data/Status/Definition）、Hook 跨组件安全、8 字段拆分阈值 | 提议 |
| `system-design-rules.md` | 参数上限 8 个、读写分离、禁止 System 间直接调用、命名规范、Exclusive System 使用场景 | 提议 |
| `determinism-rules.md` | ChaCha8Rng 确定性 PRNG、预生成随机流、整数精度（禁止 f32）、状态哈希、浮点污染隔离墙 | 提议 |

## 03-data-config-asset — 数据/配置/资产（9 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `content-pipeline.md` | RON→XxxDef→XxxData→Registry 数据流、异步 AssetLoader+AssetEvent 响应式管线、统一 Registry 模式 | 提议 |
| `content-data-format.md` | RON 配置契约（id/version/tags 必填）、命名空间强制 ID 格式、两阶段加载（Raw→Baked） | 提议 |
| `content-migration-design.md` | 内容格式迁移链（V1→V2→V3）、禁止版本子目录、强类型迁移函数 | 提议 |
| `config-system-design.md` | 四层配置（Engine/GameRules/UserSettings/DebugSwitch）、反上帝配置拆分、防抖写入、MOD Patch 语义深度合并 | 提议 |
| `ids-design.md` | Strong ID newtype（禁止裸 Entity/String 跨模块）、define_id! 宏、u64 哈希缓存 | 提议 |
| `asset-organization.md` | 美术资产三树分离（assets/content/src）、Content Packs 组织、外包工作流（Git LFS + 自动校验） | 提议 |
| `asset-lifecycle-rules.md` | Handle 类型选择（Strong/Weak）、分阶段卸载（每帧≤4MB）、SafeAssetRef 封装、内存预算 256MB | 提议 |
| `asset-namespace-design.md` | `base:category/name` 命名空间前缀、MOD 隔离、结构化 AssetId + AssetKey(u64) 预计算哈希 | 提议 |
| `save-migration-rules.md` | 存档 SemVer 版本号、向前兼容（新增字段=默认值）、三步删除原则、只保存 Instance | 提议 |

## 04-events-logging-error — 事件/日志/错误（3 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `events-audit-design.md` | 独立 Struct 事件 + Auditable Trait、EventOrd 同 Tick 排序、EventWhitelist 白名单、双轨制日志 | 提议 |
| `logging-design.md` | 日志是领域事件消费者（非主动调用）、五级日志体系、LogObserver 统一输出、Feature Gate 编译控制 | 提议 |
| `error-architecture.md` | 三层错误模型（Domain/Infrastructure/Shared）、失败分类学（RuleFailure/DomainError/InfrastructureError/Bug） | 提议 |

## 05-ui — UI 架构（1 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `ui-domain-boundary-rules.md` | 单向数据流（CQRS）、UI 只读 ViewModel + 只输出 UiCommand、CommandHandler 写入收口 | 提议 |

## 06-map-pathfinding — 地图/寻路（1 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `pathfinding-design.md` | PathFinder trait、UnitBlocker、移动/攻击/技能范围统一计算接口、地形消耗与缓存策略 | 提议 |

## 07-tools-testing-quality — 工具/测试/质量（5 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `tools-architecture.md` | Tools 层独立二进制（永不发布）、data_validator/content_linter/balance_checker、headless CI 模式 | 提议 |
| `testing-architecture.md` | 五层测试金字塔（单元→领域集成→系统集成→E2E→Testbeds）、Battle Replay 测试、Golden Test | 提议 |
| `validation-rules.md` | 校验检查点（回合结束/战斗结束/状态转换）、全局不变量（HP≥0/Buff 有来源/Modifier 有 Source） | 提议 |
| `performance-budget.md` | 60fps 帧预算、模块级预算、"测量优先、禁止凭直觉优化"原则 | 提议 |
| `feature-flag-design.md` | 7 个 Feature Flag（replay/debug_ui/cheat/modding/network/telemetry/profiler）、PluginGroup 模式 | 提议 |

## 08-i18n-modding-collaboration — 国际化/MOD/协作（3 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `i18n-design.md` | Fluent (.ftl) 国际化、永久唯一 ID Key、LocalizedText 组件、字体回退链、MOD 本地化隔离 | 提议 |
| `modding-design.md` | MOD 加载器、沙箱隔离、兼容性校验、分级权限策略 | 提议 |
| `collaboration-model.md` | AI 7-Agent 协作流程、Handoff 协议、CI 分级、反仓库膨胀策略 | 提议 |

## 09-infrastructure-migration — 基础设施/迁移（2 文件）

| 文件 | 核心内容 | 状态 |
|------|---------|------|
| `infrastructure-design.md` | 20 个 Infrastructure 模块（P0-P3 优先级）、模块依赖图、可替换性原则、存档 JSON→二进制预留 | 提议 |
| `migration-roadmap.md` | 7 Phase 迁移计划（Shared→Core→Infra→Content→App→Modding→Tools） | 提议 |
...
详见 `docs/01-architecture/00-overview/layer-contracts.md`。
...
详见 `docs/01-architecture/04-events-logging-error/error-architecture.md`。
...
详见 `docs/01-architecture/00-overview/project-structure.md`。
...
详见 `docs/01-architecture/09-infrastructure-migration/migration-roadmap.md`。
...
> 详见 `docs/01-architecture/00-overview/project-structure.md` 和 `docs/01-architecture/00-overview/layer-contracts.md`
...
详见 `docs/01-architecture/00-overview/app-bootstrap.md` 和 `docs/01-architecture/00-overview/plugin-design.md`。
...
> 当前 RON 配置数据存放在 `assets/` 目录下（过渡期），最终应迁移到 `content/` 目录（参见 `docs/01-architecture/09-infrastructure-migration/migration-roadmap.md` Phase 4）。`assets/definitions/` 和 `assets/rules/` 中的引擎配置是过渡产物。
...
详见 `docs/01-architecture/01-battle-gas/skill-buff-abstraction.md`。
...
详见 `docs/01-architecture/08-i18n-modding-collaboration/modding-design.md`。
