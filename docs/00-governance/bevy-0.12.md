...
![A relaxing 'tiny planet' city builder by Jos Feenstra (made with Bevy)](https://bevy.org/news/bevy-0-12/cover.gif)

[A relaxing 'tiny planet' city builder by Jos Feenstra (made with Bevy)](https://twitter.com/i_am_feenster)

Thanks to **185** contributors, **567** pull requests, community reviewers, and our [**generous sponsors**](https://bevy.org/donate), we're happy to announce the **Bevy 0.12** release on [crates.io](https://crates.io/crates/bevy)!
...
To update an existing Bevy App or Plugin to **Bevy 0.12**, check out our [0.11 to 0.12 Migration Guide](https://bevy.org/learn/migration-guides/0.11-0.12/).
...
## Deferred Rendering [#](https://bevy.org/news/bevy-0-12/#deferred-rendering)
...
Bevy's renderer has historically been a "forward renderer". More specifically, it is a [Clustered Forward / Forward+](https://bevy.org/news/bevy-0-7/#unlimited-point-lights) renderer, which means we break the view frustum up into clusters and assign lights to those clusters, allowing us to render many more lights than a traditional forward renderer.

However, as Bevy has grown, it has slowly moved into "hybrid renderer" territory. In previous releases, we added a [Depth and Normal Prepass](https://bevy.org/news/bevy-0-10/#depth-and-normal-prepass) to enable [TAA](https://bevy.org/news/bevy-0-11/#temporal-anti-aliasing), [SSAO](https://bevy.org/news/bevy-0-11/#screen-space-ambient-occlusion), and [Alpha Texture Shadow Maps](https://bevy.org/news/bevy-0-10/#shadow-mapping-using-prepass-shaders). We also added a Motion Vector Prepass to enable TAA.

In **Bevy 0.12** we added optional support for Deferred Rendering (building on the existing prepass work). Each material can choose whether it will go through the forward or deferred path, and this can be configured per-material-instance. Bevy also has a new [`DefaultOpaqueRendererMethod`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.DefaultOpaqueRendererMethod.html) resource, which configures the global default. This is set to "forward" by default. The global default can be overridden per-material.

Lets break down the components of this deferred render:

![deferred](https://bevy.org/news/bevy-0-12/deferred.png)

When deferred is enabled for the PBR [`StandardMaterial`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.StandardMaterial.html), the deferred prepass packs PBR information into the Gbuffer, which can be broken up into:

**Base Color** ![base color](https://bevy.org/news/bevy-0-12/base_color.png)

**Depth** ![depth](https://bevy.org/news/bevy-0-12/depth.png)

**Normals** ![normals](https://bevy.org/news/bevy-0-12/normals.png)

**Perceptual Roughness** ![perceptual roughness](https://bevy.org/news/bevy-0-12/perceptual_roughness.png)

**Metallic** ![metallic](https://bevy.org/news/bevy-0-12/metallic.png)
...
## PCF Shadow Filtering [#](https://bevy.org/news/bevy-0-12/#pcf-shadow-filtering)

authors: @superdump (Rob Swain), @JMS55
...
We also implemented the [`ShadowMapFilter::Jimenez14`](https://docs.rs/bevy/0.12.0/bevy/pbr/enum.ShadowFilteringMethod.html#variant.Jimenez14) method by Jorge Jimenez (used in Call of Duty Advanced Warfare). This can be slightly cheaper than Castano, but it can flicker. It benefits from [Temporal Anti-Aliasing (TAA)](https://bevy.org/news/bevy-0-11/#temporal-anti-aliasing) which can reduce the flickering. It can also blend shadow cascades a bit more smoothly than Castano.

Drag this image to compare

![Jimenez 14 PCF](https://bevy.org/news/bevy-0-12/pcf_jimenez.png)![PCF Off](https://bevy.org/news/bevy-0-12/no_pcf.png)

## `StandardMaterial` Light Transmission [#](https://bevy.org/news/bevy-0-12/#standardmaterial-light-transmission)

author: Marco Buono (@coreh)

The [`StandardMaterial`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.StandardMaterial.html) now supports a number of light transmission-related properties:
...
To complement the new transmission properties, a new [`TransmittedShadowReceiver`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.TransmittedShadowReceiver.html) component has been introduced, which can be added to entities with diffuse transmissive materials to receive shadows cast from the opposite side of the mesh. This is most useful for rendering thin, two-sided translucent objects like tree leaves or paper.

Additionally, two extra fields have been added to the [`Camera3d`](https://docs.rs/bevy/0.12.0/bevy/core_pipeline/core_3d/struct.Camera3d.html) component: `screen_space_specular_transmission_quality` and `screen_space_specular_transmission_steps`. These are used to control the quality of the screen-space specular transmission effect (number of taps), and how many “layers of transparency” are supported when multiple transmissive objects are in front of each other.
...
### Compatibility [#](https://bevy.org/news/bevy-0-12/#compatibility)
...
[`DepthPrepass`](https://docs.rs/bevy/0.12.0/bevy/core_pipeline/prepass/struct.DepthPrepass.html) and TAA can greatly improve the quality of the screen-space specular transmission effect, and are recommended to be used with it, on the platforms where they are supported.

### Implementation Details [#](https://bevy.org/news/bevy-0-12/#implementation-details)
...
Most existing user-facing asset code will either require no changes at all, or minimal changes. Custom [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) or [`AssetReader`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetReader.html) code will need to change slightly, but generally the changes should be very minimal. Bevy Asset V2 (despite being a completely new implementation) largely just expands what Bevy is capable of.

### Asset Preprocessing [#](https://bevy.org/news/bevy-0-12/#asset-preprocessing)
...
### Enabling Pre-Processing [#](https://bevy.org/news/bevy-0-12/#enabling-pre-processing)

To enable asset pre-processing, just configure your [`AssetPlugin`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPlugin.html) like this:
...
This will start the [`AssetProcessor`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.AssetProcessor.html) in parallel with your app. It will run until all assets are read from their source (defaults to the `assets` folder), processed, and the results have been written to their destination (defaults to the `imported_assets` folder). This pairs with asset hot-reloading. If you make a change, this will be detected by the [`AssetProcessor`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.AssetProcessor.html), the asset will be reprocessed, and the result will be hot-reloaded in your app.

### Should You Enable Pre-Processing Today? [#](https://bevy.org/news/bevy-0-12/#should-you-enable-pre-processing-today)
...
1. Most of our built-in assets don't have processors implemented for them yet. The [`CompressedImageSaver`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.CompressedImageSaver.html) is the only built-in processor and it has a bare-minimum set of features.
...
### Incremental and Dependency Aware [#](https://bevy.org/news/bevy-0-12/#incremental-and-dependency-aware)
...
### Transactional and Reliable [#](https://bevy.org/news/bevy-0-12/#transactional-and-reliable)
...
The [`AssetProcessor`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.AssetProcessor.html) can close (either intentionally or unintentionally) at any point in time and it will pick up right where it left off!
...
### Asset Meta Files [#](https://bevy.org/news/bevy-0-12/#asset-meta-files)
...
- **[`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) settings**
    - You can use meta files to set any [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) you want
    - Configure loader settings like "how to filter an image", "adjusting the up axis in 3D scenes", etc
- **[`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html) settings** (if using the `Process` action)
    - You can use meta files to set any [`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html) implementation you want
...
Note that the `Process` asset mode has changed to `Load`. This is because in the released app, we will load the final processed image "normally" like any other image asset. Note that in this case, the input and the output asset _both_ use [`ImageLoader`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.ImageLoader.html). However the processed asset _can_ use a different loader if the context demands it. Also note the addition of the `processed_info` metadata, which is used to determine if an asset needs to be reprocessed.
...
Sponza scene with textures processed into Basis Universal (with mipmaps) using Bevy Asset V2

**Bevy 0.12** ships with a barebones [`CompressedImageSaver`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.CompressedImageSaver.html) that writes images to [Basis Universal](https://github.com/BinomialLLC/basis_universal) (a GPU-friendly image interchange format) and generates [mipmaps](https://en.wikipedia.org/wiki/Mipmap). Mipmaps reduce aliasing artifacts when sampling images from different distances. This fills an important gap, as Bevy previously had no way to generate mipmaps on its own (it relied on external tooling). This can be enabled with the `basis-universal` cargo feature.

### Preprocessing is Optional! [#](https://bevy.org/news/bevy-0-12/#preprocessing-is-optional)

Despite eventually ([in future Bevy releases](https://bevy.org/news/bevy-0-12/#should-you-enable-pre-processing-today)) recommending that most people enable asset processing, we also acknowledge that Bevy is used in a variety of applications. Asset processing introduces additional complexity and workflow changes that some people will not want!

This is why Bevy offers two asset modes:

- [`AssetMode::Unprocessed`](https://docs.rs/bevy/0.12.0/bevy/asset/enum.AssetMode.html): Assets will be loaded directly from the asset source folder (defaults to `assets`) without any preprocessing. They are assumed to be in their "final format". This is the mode/workflow Bevy users are currently used to.
- [`AssetMode::Processed`](https://docs.rs/bevy/0.12.0/bevy/asset/enum.AssetMode.html): Assets will be pre-processed at development time. They will be read from their source folder (defaults to `assets`) and then written to their destination folder (defaults to `imported_assets`).

To enable this, Bevy uses a novel approach to assets: the difference between a processed and unprocessed asset is perspective. They both use the same `.meta` format and they use the same [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) interface.

A [`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html) implementation can be defined using arbitrary logic, but we heavily encourage using the [`LoadAndSave`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.LoadAndSave.html) [`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html) implementation. [`LoadAndSave`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.LoadAndSave.html) takes any [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) and passes the results to an [`AssetSaver`](https://docs.rs/bevy/0.12.0/bevy/asset/saver/trait.AssetSaver.html).

That means if you already have an [`ImageLoader`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.ImageLoader.html), which loads images, all you need to do is write some `ImageSaver` which will write the image in some optimized format. This both saves development work and makes it easy to support both processed and unprocessed scenarios.

### Built To Run Anywhere [#](https://bevy.org/news/bevy-0-12/#built-to-run-anywhere)

Unlike many other asset processors in the gamedev space, Bevy Asset V2's [`AssetProcessor`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.AssetProcessor.html) is intentionally architected to run on any platform. It doesn't use platform-limited databases or require the ability/permission to run a networked server. It can be deployed alongside a released app if your application logic requires processing at runtime.
...
### Recursive Asset Dependency Load Events [#](https://bevy.org/news/bevy-0-12/#recursive-asset-dependency-load-events)

The [`AssetEvent`](https://docs.rs/bevy/0.12.0/bevy/asset/enum.AssetEvent.html) enum now has an [`AssetEvent::LoadedWithDependencies`](https://docs.rs/bevy/0.12.0/bevy/asset/enum.AssetEvent.html) variant. This is emitted when an [`Asset`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.Asset.html), its dependencies, and all descendant / recursive dependencies have loaded.

This makes it easy to wait until an [`Asset`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.Asset.html) is "fully loaded" before doing something.

### Multiple Asset Sources [#](https://bevy.org/news/bevy-0-12/#multiple-asset-sources)

It is now possible to register more than one [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) (which replaces the old monolithic "asset provider" system).

Loading from the "default" [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) looks exactly like it does in previous Bevy versions:

```rust
sprite.texture = assets.load("path/to/sprite.png");
```

But in **Bevy 0.12** you can now register named [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) entries. For example you could register a `remote` [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) that loads assets from an HTTP server:
...
You can register a new [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) like this:
...
### Embedded Assets [#](https://bevy.org/news/bevy-0-12/#embedded-assets)
...
This required a lot of boilerplate and didn't integrate cleanly with the rest of the asset system. The [`AssetServer`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetServer.html) was not aware of these assets, hot-reloading required a special-cased second [`AssetServer`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetServer.html), and you couldn't load assets using an [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) (they had to be constructed in memory). Not ideal!

To prove out the **Multiple Asset Sources** implementation, we built a new `embedded` [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html), which replaces the old `load_internal_asset!` system with something that naturally fits into the asset system:
...
### Extendable [#](https://bevy.org/news/bevy-0-12/#extendable)

Almost everything in **Bevy Asset V2** can be extended with trait impls:

- **[`Asset`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.Asset.html)**: Define new asset types
- **[`AssetReader`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetReader.html)**: Define custom [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) read logic
- **[`AssetWriter`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetWriter.html)**: Define custom [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) write logic
- **[`AssetWatcher`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetWatcher.html)**: Define custom [`AssetSource`](https://docs.rs/bevy/0.12.0/bevy/asset/io/struct.AssetSource.html) watching / hot-reloading logic
- **[`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html)**: Define custom load logic for a given [`Asset`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.Asset.html) type
- **[`AssetSaver`](https://docs.rs/bevy/0.12.0/bevy/asset/saver/trait.AssetSaver.html)**: Define custom save logic for a given [`Asset`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.Asset.html) type
- **[`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html)**: Define fully bespoke processor logic (or use the more opinionated [`LoadAndSave`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.LoadAndSave.html) [`Process`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/trait.Process.html) impl)

### Async Asset I/O [#](https://bevy.org/news/bevy-0-12/#async-asset-i-o)

The new [`AssetReader`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetReader.html) and [`AssetWriter`](https://docs.rs/bevy/0.12.0/bevy/asset/io/trait.AssetWriter.html) APIs are async! This means naturally async backends (like networked APIs) can directly call `await` on futures.

The filesystem impls (such as [`FileAssetReader`](https://docs.rs/bevy/0.12.0/bevy/asset/io/file/struct.FileAssetReader.html)) offload file IO to a separate thread and the future resolves when the file operation has finished.

### Improved Hot-Reloading Workflow [#](https://bevy.org/news/bevy-0-12/#improved-hot-reloading-workflow)
...
### Better Asset Handles [#](https://bevy.org/news/bevy-0-12/#better-asset-handles)

Asset handles now use a single [`Arc`](https://doc.rust-lang.org/std/sync/struct.Arc.html) at their core to manage the lifetime of an asset. This simplifies the internals significantly and also enables us to make more asset information available directly from handles.

Notably, in **Bevy 0.12** we use this to provide direct [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) access from the [`Handle`](https://docs.rs/bevy/0.12.0/bevy/asset/enum.Handle.html):
...
Handles now also use a smaller / cheaper-to-look-up [`AssetIndex`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetIndex.html) internally, which uses generational indices to look up assets in dense storage.

### True Copy-on-Write Asset Paths [#](https://bevy.org/news/bevy-0-12/#true-copy-on-write-asset-paths)

The [`AssetServer`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetServer.html) and [`AssetProcessor`](https://docs.rs/bevy/0.12.0/bevy/asset/processor/struct.AssetProcessor.html) do a lot of [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) cloning (across many threads). In previous versions of Bevy, [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) was backed by Rust's [`Cow`](https://doc.rust-lang.org/std/borrow/enum.Cow.html) type. However in Rust, cloning an "owned" [`Cow`](https://doc.rust-lang.org/std/borrow/enum.Cow.html) results in a clone of the internal value. This is _not_ the "clone on write" behavior we want for [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html). We use [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) across threads, so we _need_ to start with an "owned" value.

To prevent all of this cloning and re-allocating of strings, we've built our own [`CowArc`](https://docs.rs/bevy/0.12.0/bevy/utils/enum.CowArc.html) type, which [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) uses internally. It has two tricks up its sleeve:

1. The "owned" variant is an `Arc<str>`, which we can cheaply clone without reallocating the string.
2. Almost _all_ [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html) values defined in code come from a `&'static str`. We've created a special [`CowArc::Static`](https://docs.rs/bevy/0.12.0/bevy/utils/enum.CowArc.html#variant.Static) variant that retains this static-ness, meaning we do _zero_ allocations even when turning a borrow into an "owned [`AssetPath`](https://docs.rs/bevy/0.12.0/bevy/asset/struct.AssetPath.html)".

## Suspend and Resume on Android [#](https://bevy.org/news/bevy-0-12/#suspend-and-resume-on-android)
...
Background tasks working in other threads, like playing audio, won't be stopped. When the application will be suspended, a [`Lifetime`](https://docs.rs/bevy/0.12.0/bevy/window/enum.Lifetime.html) event `ApplicationLifetime::Suspended` is sent, corresponding to the [`onStop()`](https://developer.android.com/reference/android/app/Activity#onStop\(\)) callback. You should take care to pause tasks that shouldn't run in the background, and resume them when receiving the `ApplicationLifetime::Resumed` event (corresponding to the [`onRestart()`](https://developer.android.com/reference/android/app/Activity#onRestart\(\)) callback).
...
This is accomplished via a new [`ExtendedMaterial`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.ExtendedMaterial.html) type:
...
We also paired this with some [`StandardMaterial`](https://docs.rs/bevy/0.12.0/bevy/pbr/struct.StandardMaterial.html) shader refactors to make it much easier to pick and choose which parts you want:
...
## Automatic Batching and Instancing of Draw Commands [#](https://bevy.org/news/bevy-0-12/#automatic-batching-and-instancing-of-draw-commands)
...
### 2D Mesh Bevymark (frames per second, more is better) [#](https://bevy.org/news/bevy-0-12/#2d-mesh-bevymark-frames-per-second-more-is-better)
...
### 3D Mesh "Many Cubes" (frames per second, more is better) [#](https://bevy.org/news/bevy-0-12/#3d-mesh-many-cubes-frames-per-second-more-is-better)
...
### What can be batched? [#](https://bevy.org/news/bevy-0-12/#what-can-be-batched)
...
### Opting Out [#](https://bevy.org/news/bevy-0-12/#opting-out)

If you would like to opt out an entity from automatic batching, you can add the new [`NoAutomaticBatching`](https://docs.rs/bevy/0.12.0/bevy/render/batching/struct.NoAutomaticBatching.html) component to it.
...
## The Road to GPU-driven Rendering [#](https://bevy.org/news/bevy-0-12/#the-road-to-gpu-driven-rendering)
...
### What are CPU and GPU-driven rendering? [#](https://bevy.org/news/bevy-0-12/#what-are-cpu-and-gpu-driven-rendering)
...
### What needs to change? [#](https://bevy.org/news/bevy-0-12/#what-needs-to-change)
...
In **Bevy 0.12** we've started this process in earnest! We've made a number of architectural changes that are already yielding fruit. Thanks to these changes, we can now [automatically batch and instance draws](https://bevy.org/news/bevy-0-12/#automatic-batching-and-instancing-of-draw-commands) for entities with the exact same mesh and material. And as we progress further down this path, we can batch/instance across a wider variety of cases, cutting out more and more CPU work until eventually we are "fully GPU-driven".

### Reorder Render Sets [#](https://bevy.org/news/bevy-0-12/#reorder-render-sets)
...
### BatchedUniformBuffer and GpuArrayBuffer [#](https://bevy.org/news/bevy-0-12/#batcheduniformbuffer-and-gpuarraybuffer)
...
To make it as easy as possible to support both storage types, we developed [`GpuArrayBuffer`](https://docs.rs/bevy/0.12.0/bevy/render/render_resource/enum.GpuArrayBuffer.html). This is a generic collection of `T` values that abstracts over `BatchedUniformBuffer` and [`StorageBuffer`](https://docs.rs/bevy/0.12.0/bevy/render/render_resource/struct.StorageBuffer.html). It will use the right storage for the current platform / GPU.

The data in a [`StorageBuffer`](https://docs.rs/bevy/0.12.0/bevy/render/render_resource/struct.StorageBuffer.html) looks like this:
...
[Check out this annotated code example](https://gist.github.com/cart/3a9f190bd5e789a7d42317c28843ffca) that illustrates using [`GpuArrayBuffer`](https://docs.rs/bevy/0.12.0/bevy/render/render_resource/enum.GpuArrayBuffer.html) to support both uniform and storage buffer bindings.

### 2D / 3D Mesh Entities using GpuArrayBuffer [#](https://bevy.org/news/bevy-0-12/#2d-3d-mesh-entities-using-gpuarraybuffer)

authors: @superdump (Rob Swain), @robtfm, @Elabajaba

The 2D and 3D mesh entity rendering was migrated to use [`GpuArrayBuffer`](https://docs.rs/bevy/0.12.0/bevy/render/render_resource/enum.GpuArrayBuffer.html) for the mesh uniform data.

Just avoiding the rebinding of the mesh uniform data buffer gives about a 6% increase in frame rates!

## EntityHashMap Renderer Optimization [#](https://bevy.org/news/bevy-0-12/#entityhashmap-renderer-optimization)

authors: @superdump (Rob Swain), @robtfm, @pcwalton, @jancespivo, @SkiFire13, @nicopap

Since **Bevy 0.6**, Bevy's renderer has extracted data from the "main world" into a separate "render world". This enables [Pipelined Rendering](https://bevy.org/news/bevy-0-6/#pipelined-rendering-extract-prepare-queue-render), which renders frame N in the render app, while the main app simulates frame N+1.
...
### The Path Forward [#](https://bevy.org/news/bevy-0-12/#the-path-forward)
...
We landed on using `HashMap<Entity, T>` with an optimized hash function designed by @SkiFire13, and inspired by [`rustc-hash`](https://github.com/rust-lang/rustc-hash). This is exposed as [`EntityHashMap`](https://docs.rs/bevy/0.12.0/bevy/utils/type.EntityHashMap.html) and is the new way to store component data in the render world.

This [yielded significant performance wins](https://github.com/bevyengine/bevy/pull/9903).

### Usage [#](https://bevy.org/news/bevy-0-12/#usage)

The easiest way to use it is to use the new [`ExtractInstancesPlugin`](https://docs.rs/bevy/0.12.0/bevy/render/extract_instances/struct.ExtractInstancesPlugin.html). This will extract all entities matching a query, or only those that are visible, extracting multiple components at once into one target type.
...
## Sprite Instancing [#](https://bevy.org/news/bevy-0-12/#sprite-instancing)
...
## Rusty Shader Imports [#](https://bevy.org/news/bevy-0-12/#rusty-shader-imports)
...
## glTF Emissive Strength [#](https://bevy.org/news/bevy-0-12/#gltf-emissive-strength)
...
## Import Second UV Map In glTF Files [#](https://bevy.org/news/bevy-0-12/#import-second-uv-map-in-gltf-files)
...
## Wireframe Improvements [#](https://bevy.org/news/bevy-0-12/#wireframe-improvements)

authors: @IceSentry

The wireframes now use Bevy's [`Material`](https://docs.rs/bevy/0.12.0/bevy/pbr/trait.Material.html) abstraction. This means it will automatically use the new batching and instancing features while being easier to maintain. This change also made it easier to add support for colored wireframe. You can configure the color globally or per mesh using the [`WireframeColor`](https://docs.rs/bevy/0.12.0/bevy/pbr/wireframe/struct.WireframeColor.html) component. It's also now possible to disable wireframe rendering by using the [`NoWireframe`](https://docs.rs/bevy/0.12.0/bevy/pbr/wireframe/struct.NoWireframe.html) component.

![wireframe](https://bevy.org/news/bevy-0-12/wireframe.png)

## External Renderer Context [#](https://bevy.org/news/bevy-0-12/#external-renderer-context)

authors: @awtterpip

Historically Bevy's [`RenderPlugin`](https://docs.rs/bevy/0.12.0/bevy/render/struct.RenderPlugin.html) has been fully responsible for initializing the [`wgpu`](https://github.com/gfx-rs/wgpu) render context. However some 3rd party Bevy Plugins, such as this work-in-progress [`bevy_openxr`](https://github.com/awtterpip/bevy_openxr) plugin, require more control over renderer initialization.

Therefore in **Bevy 0.12**, we've made it possible to pass in the [`wgpu`](https://github.com/gfx-rs/wgpu) render context at startup. This means the 3rd party [`bevy_openxr`](https://github.com/awtterpip/bevy_openxr/) plugin can be a "normal" Bevy plugin without needing to fork Bevy!

Here is a quick video of Bevy VR, courtesy of [`bevy_openxr`](https://github.com/awtterpip/bevy_openxr/)!

## Bind Group Ergonomics [#](https://bevy.org/news/bevy-0-12/#bind-group-ergonomics)
...
**Bevy 0.12** adds a new [`system.map()`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/trait.IntoSystem.html#method.map) function, which is a cheaper and more ergonomic alternative to [`system.pipe()`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/trait.IntoSystem.html#method.pipe).

Unlike [`system.pipe()`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/trait.IntoSystem.html#method.pipe), [`system.map()`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/trait.IntoSystem.html#method.map) just takes a normal closure (instead of another system) that accepts the output of the system as its parameter:
...
Bevy provides built in `error`, `warn`, `debug`, and `info` adapters that can be used with [`system.map()`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/trait.IntoSystem.html#method.map) to log errors at each of these levels.

## Simplify Parallel Iteration Method [#](https://bevy.org/news/bevy-0-12/#simplify-parallel-iteration-method)

authors: @JoJoJet

**Bevy 0.12** makes the parallel Query iterator [`for_each()`](https://docs.rs/bevy/0.12.0/bevy/ecs/query/struct.QueryParIter.html#method.for_each) compatible with both mutable and immutable queries, reducing API surface and removing the need to write `mut` twice:
...
## Disjoint Mutable World Access Via EntityMut [#](https://bevy.org/news/bevy-0-12/#disjoint-mutable-world-access-via-entitymut)

authors: @JoJoJet

**Bevy 0.12** supports safely accessing multiple [`EntityMut`](https://docs.rs/bevy/0.12.0/bevy/ecs/world/struct.EntityMut.html) values at once, meaning you can mutate multiple entities (with access to _all of their components_) at the same time.
...
This required reducing the access scope of [`EntityMut`](https://docs.rs/bevy/0.12.0/bevy/ecs/world/struct.EntityMut.html) to _only_ the entity it accesses (previously it had escape hatches that allowed direct [`World`](https://docs.rs/bevy/0.12.0/bevy/ecs/world/struct.World.html) access). Use [`EntityWorldMut`](https://docs.rs/bevy/0.12.0/bevy/ecs/world/struct.EntityWorldMut.html) for an equivalent to the old "global access" approach.

## Unified configure_sets API [#](https://bevy.org/news/bevy-0-12/#unified-configure-sets-api)

authors: @geieredgar

Bevy's [Schedule-First API](https://bevy.org/news/bevy-0-11/#schedule-first-ecs-apis) introduced in **Bevy 0.11** unified most of the ECS scheduler API surface under a single `add_systems` API. However, we didn't do a unified API for `configure_sets`, meaning there were two different APIs:
...
## UI Materials [#](https://bevy.org/news/bevy-0-12/#ui-materials)

authors: @MarkusTheOrt

Bevy's material system has been brought to Bevy UI thanks to the new [`UiMaterial`](https://docs.rs/bevy/0.12.0/bevy/ui/trait.UiMaterial.html):
...
## UI Node Outlines [#](https://bevy.org/news/bevy-0-12/#ui-node-outlines)

authors: @ickshonpe

Bevy's UI nodes now support outlines "outside the borders" of UI nodes via the new [`Outline`](https://docs.rs/bevy/0.12.0/bevy/ui/struct.Outline.html) component. [`Outline`](https://docs.rs/bevy/0.12.0/bevy/ui/struct.Outline.html) does not occupy any space in the layout. This is different than [`Style::border`](https://docs.rs/bevy/0.12.0/bevy/ui/struct.Style.html), which exists "as part of" the node in the layout:
...
## Unified `Time` [#](https://bevy.org/news/bevy-0-12/#unified-time)

authors: @nakedible @maniwani @alice-i-cecile

Bevy 0.12 brings two major quality of life improvements to [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html).

- [`Time`](https://docs.rs/bevy/0.12.0/bevy/time/struct.Time.html) now returns the contextually correct values for systems running in [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html). (As such, `FixedTime` has been removed.)
- [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html) can no longer snowball into a "death spiral" (where the app freezes because [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html) steps are enqueued faster than it can run them).

The [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html) schedule and its companion `FixedTime` resource were introduced in Bevy 0.10, and it soon became apparent that `FixedTime` was lacking. Its methods were different from [`Time`](https://docs.rs/bevy/0.12.0/bevy/time/struct.Time.html) and it didn't even track "total time elapsed" like [`Time`](https://docs.rs/bevy/0.12.0/bevy/time/struct.Time.html) did, to name a few examples. Having two different "time" APIs also meant you had to write systems to specifically support "fixed timestep" or "variable timestep" and not both. It was desirable to not have this split as it can lead to incompatibilities between plugins down the road (which is sometimes the case with plugins in other game engines).

Now, you can just write systems that read [`Time`](https://docs.rs/bevy/0.12.0/bevy/time/struct.Time.html) and schedule them in either context.
...
Most systems should continue to use [`Time`](https://docs.rs/bevy/0.12.0/bevy/time/struct.Time.html), but behind the scenes, the methods from previous APIs have been refactored into four clocks:
...
The fix for the windup problem was limiting how much `Time<Virtual>` can advance from a single frame. This then limits how many times [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html) can be queued for the next frame, and so things like frame lag or your computer waking up from a long sleep can no longer cause a death spiral. So now, the app won't freeze, but things happening in [`FixedUpdate`](https://docs.rs/bevy/0.12.0/bevy/app/struct.FixedUpdate.html) will appear to slow down since it'll be running at a temporarily reduced rate.

## ImageLoader Settings [#](https://bevy.org/news/bevy-0-12/#imageloader-settings)

authors: @cart, @Kanabenki

To take advantage of the new [`AssetLoader`](https://docs.rs/bevy/0.12.0/bevy/asset/trait.AssetLoader.html) settings in **Bevy Asset V2**, we've added [`ImageLoaderSettings`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.ImageLoaderSettings.html) to [`ImageLoader`](https://docs.rs/bevy/0.12.0/bevy/render/texture/struct.ImageLoader.html).
...
When set to `Default`, the image will use whatever is configured in [`ImagePlugin::default_sampler`](https://docs.rs/bevy/0.12.0/bevy/render/prelude/struct.ImagePlugin.html#structfield.default_sampler).
...
- The [`Input`](https://docs.rs/bevy/0.12.0/bevy/input/struct.Input.html) Resource: read the _current_ state of the input

One notable exception was [`GamepadButton`](https://docs.rs/bevy/0.12.0/bevy/input/gamepad/struct.GamepadButton.html), which was only available via the [`Input`](https://docs.rs/bevy/0.12.0/bevy/input/struct.Input.html) resource. **Bevy 0.12** adds a new [`GamepadButtonInput`](https://docs.rs/bevy/0.12.0/bevy/input/gamepad/struct.GamepadButtonInput.html) event, filling this gap.

## SceneInstanceReady Event [#](https://bevy.org/news/bevy-0-12/#sceneinstanceready-event)

authors: @Shatur

**Bevy 0.12** adds a new [`SceneInstanceReady`](https://docs.rs/bevy/0.12.0/bevy/scene/struct.SceneInstanceReady.html) event, which makes it easy to listen for a specific scene instance to be ready. "Ready" in this case means "fully spawned as an entity".
...
## Split Computed Visibility [#](https://bevy.org/news/bevy-0-12/#split-computed-visibility)

authors: @JoJoJet

The `ComputedVisibility` component has now been split into [`InheritedVisibility`](https://docs.rs/bevy/0.12.0/bevy/render/view/struct.InheritedVisibility.html) (visible in the hierarchy) and [`ViewVisibility`](https://docs.rs/bevy/0.12.0/bevy/render/view/struct.ViewVisibility.html) (visible from a view), making it possible to use Bevy's built-in change detection on both sets of data separately.

## ReflectBundle [#](https://bevy.org/news/bevy-0-12/#reflectbundle)

authors: @Shatur

Bevy now supports "Bundle reflection" via [`ReflectBundle`](https://docs.rs/bevy/0.12.0/bevy/ecs/reflect/struct.ReflectBundle.html):
...
It is now possible to insert and remove reflect components from an entity in a normal system via new functions on [`Commands`](https://docs.rs/bevy/0.12.0/bevy/ecs/system/struct.Commands.html)!
...
The above commands use the [`AppTypeRegistry`](https://docs.rs/bevy/0.12.0/bevy/ecs/reflect/struct.AppTypeRegistry.html) by default. If you use a different TypeRegistry then you can use the ...`with_registry` commands instead.
...
See [`ReflectCommandExt`](https://docs.rs/bevy/0.12.0/bevy/ecs/reflect/trait.ReflectCommandExt.html) for more examples and documentation

## Limit Background FPS [#](https://bevy.org/news/bevy-0-12/#limit-background-fps)
...
## `AnimationPlayer` API Improvements [#](https://bevy.org/news/bevy-0-12/#animationplayer-api-improvements)
...
## Ignore Ambiguous Components and Resources [#](https://bevy.org/news/bevy-0-12/#ignore-ambiguous-components-and-resources)
...
## Spatial Audio API Ergonomics [#](https://bevy.org/news/bevy-0-12/#spatial-audio-api-ergonomics)
...
## Pitch Audio Source [#](https://bevy.org/news/bevy-0-12/#pitch-audio-source)
...
## Added HSL methods to `Color` struct [#](https://bevy.org/news/bevy-0-12/#added-hsl-methods-to-color-struct)
...
## Reduced Tracing Overhead [#](https://bevy.org/news/bevy-0-12/#reduced-tracing-overhead)
...
## AccessKit Integration Improvements [#](https://bevy.org/news/bevy-0-12/#accesskit-integration-improvements)
...
## TypePath Migration [#](https://bevy.org/news/bevy-0-12/#typepath-migration)

authors: @soqb

As a followup to the introduction of [Stable TypePath](https://bevy.org/news/bevy-0-11/#stable-typepath) in **Bevy 0.11**, Bevy Reflect now uses [`TypePath`](https://docs.rs/bevy/0.12.0/bevy/reflect/trait.TypePath.html) instead of [`type_name`](https://doc.rust-lang.org/std/any/fn.type_name.html). A reflected type's [`TypePath`](https://docs.rs/bevy/0.12.0/bevy/reflect/trait.TypePath.html) is now accessible via [`TypeInfo`](https://docs.rs/bevy/0.12.0/bevy/reflect/enum.TypeInfo.html) and [`DynamicTypePath`](https://docs.rs/bevy/0.12.0/bevy/reflect/trait.DynamicTypePath.html) and [`type_name`](https://doc.rust-lang.org/std/any/fn.type_name.html) methods have been removed.

## Improved bevymark Example [#](https://bevy.org/news/bevy-0-12/#improved-bevymark-example)
...
## CI Improvements [#](https://bevy.org/news/bevy-0-12/#ci-improvements)
...
## Example tooling improvements [#](https://bevy.org/news/bevy-0-12/#example-tooling-improvements)

authors: @mockersf

The example showcase tool can now build all examples for WebGL2 or WebGPU. This is used to update the website with all Wasm-compatible examples, which you can find [here](https://bevy.org/examples/) for WebGL2, or [here](https://bevy.org/examples-webgpu/) for WebGPU.
...
## Example execution in CI [#](https://bevy.org/news/bevy-0-12/#example-execution-in-ci)
...
A report of all those executions is built and available [here](https://thebevyflock.github.io/bevy-example-runner/).
...
## What's Next? [#](https://bevy.org/news/bevy-0-12/#what-s-next)
