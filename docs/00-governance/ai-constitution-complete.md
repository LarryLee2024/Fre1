...
| 层级 | 名称 | 核心定位 | 依赖权限 |
|------|------|----------|----------|
| **L0** | Shared | 全局基础原子能力，零业务语义 | 零外部依赖（依赖图叶子节点） |
...
- 🟩 **唯一通信方式**：跨 Domain 交互只能通过全局领域事件（Message）广播
...
- Domain A → Domain B：❌ 绝对禁止，只能通过事件通信
...
#### 测试金字塔
- **单元测试**：验证领域纯函数、核心规则
- **集成测试**：验证完整 Feature 的端到端行为
- **回放测试**：通过 Battle Replay 验证完整战斗场景的确定性
...
26. Domain 之间直接 `use` 对方内部类型，绕过事件通信

### 20.2 AI 代码自检清单（强制执行）
生成任何代码后，必须自动完成检查并在代码开头标注结果：
```rust
// ================================================
// SRPG 项目总宪法 v5.0 自检结果
// ================================================
// ✅ 按业务拆分模块，无全局技术型巨文件：是
// ✅ 配置与运行时分离：是
// ✅ 逻辑与表现分离：是
...
// ✅ 区分规则失败与程序错误，语义正确：是
// ✅ 双轴边界合规：Capabilities无业务规则，Domain无重复机制：是
// ✅ Domain间无直接依赖，通信走事件：是
// ✅ 日志与错误使用强类型可读ID：是
// ✅ 无必修级Rust/Clippy警告：是
// ✅ 新增核心系统附带对应测试用例：是
// ✅ 新增领域事件已纳入白名单：是
// ✅ 核心领域逻辑纯函数化，不绑定引擎表现：是
// ✅ 读路径无副作用，读写分离：是
// ✅ 使用统一RNG服务，无直接随机调用：是
// ✅ 无硬编码数值魔法数字：是
