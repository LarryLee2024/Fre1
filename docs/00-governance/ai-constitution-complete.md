...
## 第二编 六层架构体系（技术边界）
### 2.1 架构总览
采用「6层物理目录 + 3层逻辑抽象」的双层架构体系，从根源避免目录爆炸、边界混乱、依赖蜘蛛网问题。

#### 逻辑三层（抽象划分）
- **领域层**：核心业务规则、数值计算、状态定义，纯逻辑无引擎表现依赖
- **应用层**：ECS调度、系统编排、状态流转、数据桥接，粘合领域逻辑与引擎
- **表现层**：UI、特效、音效、输入处理，仅读取状态不修改核心逻辑

#### 物理六层（落地目录）
从上层到下层，依赖方向严格单向，下层不得依赖上层。

| 层级 | 名称 | 核心定位 | 依赖权限 | 逻辑归属 |
|------|------|----------|----------|----------|
| Layer 1 | App | 游戏启动与全局装配 | 允许依赖所有层 | 应用层 |
| Layer 2 | Core | 纯游戏规则与领域逻辑（内部双轴结构） | 仅允许依赖 Shared + bevy_ecs 核心 | 领域层 |
| Layer 3 | Shared | 全局基础原子能力 | 零外部依赖（依赖图叶子节点） | 领域层基础 |
| Layer 4 | Infrastructure | 技术实现与引擎封装 | 允许依赖 Core + Shared | 应用层+表现层 |
| Layer 5 | Content | 数据与规则的桥接层 | 允许依赖 Core + Shared + Infrastructure | 应用层 |
| Layer 6 | Tools | 开发期工具链 | 允许依赖 Core + Shared，永不进入发布构建 | 开发期扩展 |

> Modding 不作为独立层级，而是跨层扩展能力，分散到对应层级中。

### 2.2 Layer 1：App — 启动装配层
#### 职责
...
### 2.3 Layer 2：Core — 游戏规则核心层
项目的心脏，唯一允许定义游戏规则的层级。**内部采用「Layers 能力层 + Domains 业务域」双轴结构**，兼顾通用能力复用与业务逻辑内聚。
...
### 2.5 Layer 4：Infrastructure — 技术实现层
...
#### 双类型协作模式
```
core/layers/l2_execution/ability/skill_def.rs    配置反序列化结构
core/layers/l2_execution/ability/skill_data.rs   运行时结构
content/abilities/                                RON → Def → Data → 注册
```
...
5. 存档兼容性损坏
6. 核心数据损坏、状态机非法跳转
7. 双轴边界严重突破：Layers 包含业务规则、Domain 间直接依赖
...
- [ ] 未依赖禁止的层级与模块
- [ ] 未包含不属于当前层的逻辑
- [ ] Layers 模块未包含业务规则，Domain 未重复实现通用机制
...
25. 在 Layers 层硬编码业务规则，破坏机制与业务的边界
...
// ✅ 区分规则失败与程序错误，语义正确：是
// ✅ 双轴边界合规：Layers无业务规则，Domain无重复机制：是
...
8. 🟩 双轴架构原则：Layers 管机制，Domains 管业务，边界不可突破
...
13. 🟥 禁止为临时副作用随意新增领域事件
14. 🟥 禁止凭感觉优化突破架构边界
15. 🟥 禁止 Layers 层包含具体业务规则，突破机制与业务的边界
16. 🟥 禁止 Domain 之间直接依赖、直接调用内部实现
17. 🟥 禁止 Domain 重复实现 layers 已有的通用机制

---

## 附则
### 修订说明
- 本宪法版本：v4.1（Bevy 0.18+）
- 发布日期：2026-06-16
- 核心升级（v4.0 → v4.1）：
  1. **架构体系升级**
     - Core 层从单纵向五级分层，升级为「Layers 能力层 + Domains 业务域」双轴结构
     - 明确 Layers 只提供通用机制、Domains 封装业务规则的分工原则
     - 新增 Domain 标准结构、边界铁则、交互规范，解决大项目下层内膨胀问题
  2. **编译治理强化**
     - 引入 Domain 级平行编译优化，通过横向切片降低重编译范围
     - 补充 CI 门禁中的 Domain 边界检查项
  3. **Mod API 升级**
     - 重构为 Facade + Domain Gateway 模式，对外只暴露业务语义接口
     - 隐藏内部分层结构，保证 API 长期稳定性与向后兼容
  4. **治理规则补全**
     - 新增双轴边界的强制约束与禁止事项
     - 更新 AI 反模式黑名单与自检清单，覆盖双轴架构规则
     - 补充问题分级标准，将架构边界违规纳入 P1 级强制修复项
