...
|------|------|-------------|-------------|
| IDs | `src/shared/ids/` | — | `foundation/id_strategy.md` + `foundation/id-taxonomy.md` <br> 重构计划: `docs/11-refactor/id-system-refactoring-2026-06-20.md` |
...
| `localization` | `infrastructure/localization_schema.md` | Fluent i18n 文本数据库、加载、校验、缓存、Fake Locale |
...
// LocalizationPlugin: 放在 Content Plugin（Phase 10）之后、UI Plugin 之前，
// 实际位于 Phase 8 末尾（Phase 9/10 之前），确保 Content 层配置
// 加载完成后初始化本地化资源。
// ════════════════════════════════════════════
...
// 详见 ADR-050。
.add_plugins(app::scenes::ScenePlugin) // GameState + 场景生命周期
