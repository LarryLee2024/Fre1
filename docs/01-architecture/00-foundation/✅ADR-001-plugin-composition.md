...
## DomainPlugin trait（预留）

### 现状

当前项目 62 个 Plugin 已统一使用 Bevy 标准 `Plugin` trait。标准 trait 足够满足当前需求：`fn build(&self, app: &mut App)` 提供了完整的注册入口，配合 PluginGroup 实现分组注册。

### 未来扩展场景

如果未来出现以下需求，应考虑自定义 `DomainPlugin` trait：

| 场景 | 需求 | 可能的设计 |
|------|------|-----------|
| 自动注册依赖检查 | 需要支持声明式依赖声明，而非手动检查 Resource | `trait DomainPlugin: Plugin { fn deps() -> Vec<TypeId>; }` |
| 自动接口导出 | 需要自动生成 `integration/` 模块的 API 元信息 | `trait DomainPlugin: Plugin { fn export_api() -> Vec<DomainAPI>; }` |
| 领域级配置 | 每个 Domain 需要在 build 前加载配置 | `trait DomainPlugin: Plugin { fn config() -> DomainConfig; }` |

### 当前决策

- **当前不引入自定义 `DomainPlugin` trait**，保持 Bevy 标准 `Plugin` trait
- 如果上述场景中出现 3 个以上的实际需求，再启动自定义 trait 设计
- 自定义 trait 必须保持向后兼容，允许标准 `Plugin` trait 和自定义 trait 共存

### 约束

- 🟥 禁止在当前阶段引入自定义 Plugin trait（三次才抽象原则）
- 🟥 禁止自定义 trait 隐藏或绕过标准 `Plugin::build()` 方法
- 🟩 未来设计必须同时支持自定义 trait 和标准 Plugin trait
