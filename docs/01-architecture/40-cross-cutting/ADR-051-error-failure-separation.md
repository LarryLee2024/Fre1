...
**阶段 2（待 Phase 2 执行）**：
- 逐个 system/function 将 `*Error` 中的 Failure 用法迁移到 `*Failure`
- 迁移完成后删除 `*Error` 中的重复 Failure 变体
- 清理 `*Error` 变体中 `"规则失败: "` 的前缀标注
- 确保调用方正确处理 Failure 值（不走 Err 链传播）

**阶段 2 的迁移优先级**：combat → inventory → spell → 其他 domain（按需求紧迫度排序）
...
阶段 2 完成前，`*Error` 中的重复 Failure 变体**保持不动**：
- 不删除现有 Failure 变体（不破坏既有调用方）
- 现有 Failure 变体在 `*Error` 中的 error 字符串保留 `"规则失败: "` 前缀（作为临时标注）
- 新调用优先使用 `*Failure`，`*Error` 中的副本仅供旧代码使用
...
- [ ] 符合 DDD 三层 + 横切四层架构（Failure 在 domain 内部，不走跨域通道）
- [ ] 符合 ADR-045 可见性策略（failure 模块 `pub(crate)`）
- [ ] 符合 `.trae/rules/错误规则.md` 的失败分类标准
- [ ] `RuleFailure` trait 仅含 `code()`，无 `message()` / Display bound
- [ ] Failure 值通过函数返回值传递，不走 Err / Event
- [ ] `*Error` 枚举中不再新增 Failure 语义变体
- [ ] 阶段 2 完成前保留 `*Error` 中的重复变体
- [ ] Forbidden 列表已明确列出所有禁止行为
- [ ] 与现有 ADR（045、046、049）无冲突
...
| `src/core/domains/combat/failure.rs` | ✅ stable | feature-developer | 2026-06-19 |
| `src/core/domains/inventory/failure.rs` | ✅ stable | feature-developer | 2026-06-19 |
| `src/core/domains/spell/failure.rs` | ✅ stable | feature-developer | 2026-06-19 |
| 其余 domain failure.rs | 📋 planned | feature-developer | TBD (Phase 2) |
| `*Error` 重复变体清理 | 📋 planned | feature-developer | TBD (Phase 2) |
