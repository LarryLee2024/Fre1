...
> **统一抽象模型**：Skill/Buff/Effect 的数据驱动抽象层级参见 `docs/01-architecture/01-battle-gas/skill-buff-abstraction.md`。本文件关注 Content 层如何将 RON 数据桥接到 Core 层的规则引擎。
...
> 交叉引用：`docs/01-architecture/validation_rules.md` §10（启动时校验）、`docs/01-architecture/01-battle-gas/skill-buff-abstraction.md` §6（Content/Rule 映射表）
