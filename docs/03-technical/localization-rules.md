---
id: 03-technical.localization-rules
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 3.3 — Key 永久 ID 策略
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 8.5 — MOD 覆盖链的实现细节
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 7.3 — 语音集成预留
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 6A — Cosmic Text 集成、Flexbox 弹性布局、RTL 语言支持
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 1 — 技术决策：Fluent (.ftl)
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 10A — 性能优化
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 6A — Text Overflow
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 1.1、§ 10.1 — 自封装 fluent-rs + intl-memoizer
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 2.2、§ 2.3 — 文件拆分规则
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 8.5 — MOD 覆盖链（Fallback Chain）
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 10A — FluentBundle 预解析、文本解析缓存、异步加载
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 7.1、§ 7.3 — 语音集成预留
...
> **优化来源**: `docs/01-architecture/i18n_design.md` § 11A — FTL 版本控制与翻译协作
...
| 原则 | 说明 |
|------|------|
| Key 唯一永久 | 命名空间 + 永久唯一 ID，一旦创建永不修改 |
| 配置只存 Key | 所有 Content 配置中的 name/description 替换为 name_key/desc_key |
| 禁止语义化 Key | 禁止使用 `skill.fireball.name`，必须用 `skill.s_1001.name` |
| 回退链保障 | 请求语言 → 项目默认(zh-CN) → 英语(en-US) → Key本身 |
...
| 领域 | 原字段 | 新字段 | Key 格式 | 必选 |
|------|--------|--------|----------|------|
| Attribute | `name` | `name_key` | `attr.a_XXX.name` | ✅ |
| Tag | `name` | `name_key` | `tag.t_XXX.name` | ✅ |
| Effect | `name` | `name_key` | `buff.b_XXX.name` | ✅ |
| Effect | `description` | `desc_key` | `buff.b_XXX.desc` | ❌ |
| Ability | `name` | `name_key` | `skill.s_XXX.name` | ✅ |
| Ability | `description` | `desc_key` | `skill.s_XXX.desc` | ✅ |

Modifier、Targeting、Trigger、Execution、Stacking、Pipeline/Replay 无玩家可见文本，无需国际化。
