...
> ActionQueue 的详细设计见 `docs/architecture/command_bus_design.md` §ActionQueue。
...
> Condition 已在 `docs/domain/` 中有详细领域规则定义，此处仅做上层抽象说明。实现细节参见领域文档。
...
> Modifier 的详细实现见 `docs/domain/attribute_modifier_rules.md`。
...
> Requirement 已在 `docs/domain/skill_rules.md` 中有详细领域规则定义，此处仅做上层抽象说明。实现细节参见领域文档。
...
> 交叉引用：`docs/domain/trigger_rules.md`（触发规则）、`docs/domain/stack_policy_rules.md`（Stack 解析策略）
...
> 交叉引用：`docs/domain/trigger_rules.md`（Trigger 完整定义）、`docs/domain/stack_policy_rules.md`（Stack 解析策略）
...
> **实现细节**：Tag 位掩码实现见 `docs/domain/shared_layer_rules.md`#标签
...
> 公式的详细实现见 `docs/domain/formula_rules.md` §FormulaRegistry。
...
| 系统 | 状态 | 文档 |
|------|------|------|
| Tag | ✅ 已实现 | `docs/domain/shared_layer_rules.md`（GameplayTag 位掩码）|
...
| Trigger 规则定义 | `docs/domain/trigger_rules.md` |
| Stack 解析策略 | `docs/domain/stack_policy_rules.md` |
