...
交叉引用：
- `docs/domain/shared_layer_rules.md` — Shared Event 领域定义、AuditEvent 结构、EventWhitelist
- `docs/domain/ecs_communication_rules.md` — Message 通信机制、EventBus 分发
- `docs/domain/replay_rules.md` — 回放系统如何消费审计事件
...
事件通过 Bevy 的 Message 系统广播，遵循 `docs/domain/ecs_communication_rules.md` 中定义的 Message 发送管线：
...
关键约束（参见 `docs/domain/ecs_communication_rules.md`）：
...
详细设计见 `docs/domain/replay_rules.md`。
