...
pub struct FactionDef {
    ...
    /// 阵营类型——阵营的业务分类
    ///
    /// 用于运行时逻辑分支（如：Player 阵营的实体不可被永久击杀、
    /// Temporary 阵营在特定事件后自动解散）。
    pub faction_type: FactionType,

    /// 是否为关键/剧情阵营
    ///
    /// true: 禁止声望降至敌对阈值以下，用于保护重要剧情阵营。
    /// false: 普通阵营，声望可自由变化。
...
- **`faction_type`**: 阵营的业务分类。Player 阵营的实体不可被永久击杀；Temporary 阵营在相关剧情事件后自动解散；Quest 阵营仅在特定任务期间活跃。用于运行时逻辑分支和 UI 过滤
- **`is_critical`**: 关键阵营保护标志。设为 true 时，运行时禁止该阵营的声望值降至 Hostile 以下，防止剧情重要阵营被玩家意外敌对化
...
| V7 | `faction_type` 为有效枚举值 | 必须是 FactionType 的五个变体之一 |
| V8 | `is_critical` 为 bool 类型 | 必须为 true 或 false |
| V9 | `color_hex` 格式合法（若设置） | 必须匹配 `^#[0-9A-Fa-f]{6}([0-9A-Fa-f]{2})?$`（如 `#FF0000`、`#00FF00AA`） |
