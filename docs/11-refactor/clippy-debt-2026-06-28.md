...
> **扫描工具**: `cargo clippy` | **基线**: 722 warnings | **原则**: 预留引起的 dead code 不处理
...
| 其中预留（不处理） | ~600 | ~589 |
| 真实技术债 | **~122** | **~0** (已处理) |
| 自动修复 (`--fix`) | ~130 suggestions | 已执行 |
| 手动修复 (Phase 1) | ~15 项 | 已全部执行 |
| 涉及文件数 | ~200+ | ~60+ files changed |
...
- 全部 `redundant_closure`（2 项 + 更多可能）
- 全部 `unnecessary_filter_map`（2 项）
- 全部 `manual_div_ceil / manual_is_multiple_of / manual_clamp`（~10 项）
- 全部 `unused_mut`（6 项）
- 部分 `derivable_impls`（15 项，但不一定全部）

**注意**: `cargo clippy --fix` 可能会引入不需要的 `#[allow(...)]` 属性或删除看起来 unused 但实际是 API 导出的项。建议在分支上运行并审阅 diff。

### Phase 1：手动修复（~1 小时）

| 任务 | 文件 | 估计 |
|------|------|------|
| 修复 `drop(&mut T)` → `let _ = T` | `replay/systems.rs:120` | 1m |
| 添加 `#[derive(Default)]`（15 处） | 各文件 | 5m |
| 修复 `insert_non_send_resource` | `app/plugin.rs` | 1m |
| 清理 `unused_mut` | 6 处 | 2m |
| 修复 `type_complexity` 提取 type alias | `save/save_system.rs:21` | 3m |
| 修复 `map_entry` (`contains_key` + `insert`) | 1 处 | 2m |
| 修复 `if_same_then_else` | 1 处 | 1m |

### Phase 2：架构整理（~2 小时）

| 任务 | 估计 | 风险 |
|------|------|------|
| `module_inception` 模块重命名（`rules/rules.rs` → `rules/mod.rs`） | 30m | 🟡 模块路径变更会影响导入 |
| 战斗输入系统 16 参数拆分 | 1h | 🟡 参数过多是架构信号，可提取 Command 模式 |

### Phase 3：Bevy 系统参数治理（~4 小时，低优先级）

| 任务 | 估计 | 说明 |
|------|------|------|
| Save/Load 系统 Builder 模式 | 1h | 将 `process_pending_load` 的参数分组为 struct |
| 战斗输入系统重构 | 2h | 16 参数是架构问题，建议拆分为多个 Observer |
| 回放系统参数简化 | 1h | 使用 Command 模式组织 8-9 参数 |
...
| 指标 | 当前 | Phase 0 后 | Phase 1 后 | Phase 2/3 后 |
|------|------|-----------|-----------|-------------|
| warnings | 722 | ~590 | ~575 | ~550 |
| 其中预留 | ~600 | ~580 | ~580 | ~550 |
| 真实技术债 | ~122 | ~10 | ~2 | 0 |

> **最终保留的 ~589 warnings 全部为预留 dead code + 架构设计模式（too_many_arguments / module_inception / upper_case_acronyms / collapsible_match）**，按 ADR-045 §6.2 标记为"预留非债务"。
