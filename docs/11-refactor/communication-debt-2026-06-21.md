...
> Fix 1-3 已完成。以下为未执行的剩余计划项。

### 阶段 1: Event History 实现（P2, 2天）— 未开始

| # | 操作 | 依赖 | 说明 |
|---|------|------|------|
| 1.1 | 创建 `EventStore` 结构体 + `StoredEvent` | ADR-059 §4 | RingBuffer 内存存储 |
| 1.2 | 创建 `EventStoreConfig` Resource | ADR-059 §3 | 默认容量 1000 |
| 1.3 | EventService 注入 EventStore | ADR-059 §7 | EventBus 发布路径上附加写入 |
| 1.4 | 实现 Query Interface | ADR-059 §6 | `by_correlation()` / `by_domain()` / `by_log_code()` |
| 1.5 | 集成 ObservableEvent | ADR-059 §5 | `record_fields()` 收集 |
| 1.6 | 测试覆盖 | - | EventStore 单元 + 集成测试 |
| 1.7 | ADR-059 draft → accepted | - | 状态更新 |

**文件**: `src/core/capabilities/runtime/event/foundation/event_store.rs`

### 阶段 2: Trigger 完整实现审查（P2, 1天）— 待排期

| # | 操作 | 说明 |
|---|------|------|
| 2.1 | 审计 `trigger/mechanism/evaluator.rs` | 确认 Trigger 评估逻辑完整性 |
| 2.2 | 审计 TriggerRegistry 实现 | 与 trigger_domain.md §3-5 对比 |
| 2.3 | 补充缺失的触发流程 | 如 TriggerCooldown, 频率限制 |
| 2.4 | 补充测试 | 触发评估 + 注册 + 循环检测 |

### 阶段 3: Observer 安全机制（P2, 0.5天）— 待排期

| # | 操作 | 说明 |
|---|------|------|
| 3.1 | 定义 `MAX_OBSERVER_DEPTH` 常量 | ADR-002 要求 |
| 3.2 | 审计所有 `On<T>` Observer 使用 | 检查是否有递归风险 |
| 3.3 | 添加审计日志 | Observer 深度超限时 WARN |
| 3.4 | 更新 ECS规则.md | 补充深度限制条款 |

### 阶段 4: 共享事件完整性（P2, 0.5天）— 已完成（详见本次审计报告）

| # | 操作 | 说明 | 状态 |
|---|------|------|------|
| 4.1 | 审计 `src/core/events.rs` | 验证 4 个共享事件 struct 签名 | ✅ 签名正确，与 ADR-049 定义一致 |
| 4.2 | 补全 `BattleEnded` 消费者 | ADR-049 标记待定 | ✅ 发现 `battle_logger.rs` 订阅但事件从未触发—属死代码 |
| 4.3 | 审计 shared events 发布/订阅 | 确认 ADR-049 通信图正确 | ✅ 仅 `TurnEnded` 正确桥接；`TurnStarted`、`BattleStarted`、`BattleEnded` 均未触发 |
| 4.4 | Event History ADR-059 明确 shared events 时间轴角色 | 更新 §9 | ⏳ 待排期（ADR-059 仍为 Draft） |

**审计结论**: 3/4 的共享事件未被触发，导致 `turn_logger.rs` 和 `battle_logger.rs` 中的 Observer 死代码。建议在 combat pipeline 中添加桥接触发，或将 logger 改为直接监听 combat 域内事件。

### 阶段 5: 文档对齐（P3, 0.5天）— 待排期

| # | 操作 | 说明 |
|---|------|------|
| 5.1 | ADR-002 §3 示例对齐实际 `events.rs` | 枚举 vs struct |
| 5.2 | 宪法 §6.3 更新 | 补充 Observer 深度限制 |
| 5.3 | ECS规则.md 更新 | 补充 `EventWriter` 禁令 |
| 5.4 | ADR-059 → Accepted | 实现后更新状态 |
...
| 阶段 | 内容 | 预计工时 | 风险 |
|------|------|----------|------|
| 1 | Event History 实现 | 16h | 🟡 中 — 新功能开发 |
| 2 | Trigger 审查 | 4h | 🟢 低 — 审计为主 |
| 3 | Observer 安全机制 | 2h | 🟢 低 |
| 4 | 共享事件完整性 | 2h | 🟢 低 |
| 5 | 文档对齐 | 2h | 🟢 低 |
| **合计** | | **26h** | |
...
**Communication 系统整体质量较高。** 相比 Localization（发现 P0 硬编码违规），Communication 系统无紧急问题：
...
**主要缺口**：Event History（ADR-059）设计完成但从未实现；Observer 深度限制未落地。

**是否急迫**：P2 级，建议排入后续迭代。当前无需紧急修复。
