...
use crate::core::character::{TraitCollection, TraitEffectHandlerRegistry, TraitRegistry};
...
pub fn execute_effects(world: &mut World) {
        ...
        for effect in &mut trait_effects {
            if let Some(target_tags) = world.get::<GameplayTags>(effect.target) {
                match &mut effect.data {
                    ...
                    PendingEffectData::ApplyModifier { .. } | PendingEffectData::Cleanse => {}
                }
            }
        }
    }
    ...
    for entity in all_dead_entities {
        world
            .entity_mut(entity)
            .insert(crate::core::character::Dead);
    }
