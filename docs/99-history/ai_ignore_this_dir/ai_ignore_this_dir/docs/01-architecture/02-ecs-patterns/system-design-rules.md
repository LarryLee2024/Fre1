---
id: 01-architecture.system-design-rules
title: System Design Rules
status: draft
owner: architect
created: 2026-06-14
updated: 2026-06-14
...
Related: `docs/01-architecture/README.md` ECS 章节、`docs/02-domain/ecs_communication_rules.md`
...
> ⚠️ **反 `.chain()` 规则**（宪法 2.3.8 + app-bootstrap.md）：`.chain()` 强制系统串行执行，破坏 Bevy 多线程并行优势。效果管线（Generate→Modify→Execute）应使用自定义 Schedule（EffectPipelineSchedule），而非 `.chain()`。详见 `docs/01-architecture/00-overview/app-bootstrap.md`。
...
| `docs/01-architecture/README.md` | ECS 章节（System、Resource、系统排序） |
| `docs/01-architecture/component_design_rules.md` | System 操作的 Component 设计规范 |
| `docs/02-domain/ecs_communication_rules.md` | 系统间通信方式（Message、Observer、Command） |
| `docs/01-architecture/00-overview/plugin-design.md` | System 通过 Plugin 注册 |
| `docs/01-architecture/00-overview/app-bootstrap.md` | EffectPipelineSchedule、Schedule 权责划分 |
