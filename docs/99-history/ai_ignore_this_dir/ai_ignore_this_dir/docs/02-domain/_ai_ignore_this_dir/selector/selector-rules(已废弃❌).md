...
核心原则：
- 🟩 1.4.1 Selector 只负责目标选择，不负责效果执行（纯函数，不修改状态）
- 🟩 11.7 目标解析基于施法者坐标 + TargetRule 产生目标实体列表（读路径无副作用）
- 🟩 1.1.4 Selector 与 Skill Range 配合确定影响范围（逻辑与表现分离）
...
🟩 1.4.1 技能的目标选择规则，决定技能可以作用于哪些实体。目标解析为纯函数，不修改战场状态。
...
🟩 1.4.1 Selector 从当前战场坐标 + TargetRule 产生目标实体列表的过程。🟩 11.7 纯函数，不修改战场状态（读路径无副作用）。
...
🟩 1.4.1 TargetSelector::resolve_targets() 不修改战场状态，仅读取坐标和战场数据并返回目标实体列表。🟩 11.7 读路径无副作用。
...
## 规则1：Selector 与 Effect 解耦 🟩 1.4.1
...
修改本领域代码前，必须逐项确认：
- 🟩 1.4.1 目标解析为纯函数，不依赖 ECS World
- 🟩 1.4.2 解析过程无副作用，不触发事件、不修改全局状态
- 🟩 11.7 读写分离：目标解析是读路径，无副作用
- 🟩 1.1.3 选择器定义通过 RON 配置，不硬编码选择逻辑

## 领域事件清单

本领域产生的领域事件：选择器系统为纯函数解析，不直接产生领域事件。目标选择结果由 Skill/Battle 在执行阶段产生事件。

> 🟩 2.2.7 领域事件由效果执行阶段统一产生，目标解析阶段不产生事件
